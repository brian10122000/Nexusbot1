...
// Fix WebSocket pour Node.js 20 + Supabase
const ws = require('ws');
global.WebSocket = ws;
const {
...
const SUPABASE_URL = 'https://zgmuvrtcbwnfcpmqzjhv.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpnbXV2cnRjYnduZmNwbXF6amh2Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NzY1OTA0MSwiZXhwIjoyMDkzMjM1MDQxfQ.6g1vkIybp_iyIA1IYzTuh2m52AMj1qrLe-I-MYFqgOU';
...
if (supabase) {
  ...
  console.log('⚠️ Supabase non configuré — utilisation de la mémoire locale');
}

// ── SUPABASE HELPERS ────────────────────────────────────────────
const SB = {
  // Lire toute la DB depuis Supabase au démarrage
  async loadAll() {
    if (!supabase) return false;
    try {
      // Charger economy
      const { data: eco } = await supabase.from('economy').select('*');
      if (eco) eco.forEach(r => {
        db.economy[r.user_id] = { points:r.points, xp:r.xp, level:r.level, lastDaily:r.last_daily, lastMsg:r.last_msg, totalEarned:r.total_earned, messages:r.messages };
      });

      // Charger articles
      const { data: arts } = await supabase.from('articles').select('*').order('id');
      if (arts) db.articles = arts.map(a => ({ id:a.id, name:a.name, price:a.price, description:a.description, emoji:a.emoji, stock:a.stock, link:a.link, image:a.image, category:a.category, alertThreshold:a.alert_threshold, visible:a.visible }));

      // Charger orders
      const { data: ords } = await supabase.from('orders').select('*').order('created_at', {ascending:false});
      if (ords) db.orders = ords.map(o => ({ id:o.id, userId:o.user_id, discordId:o.discord_id, username:o.username, articleId:o.article_id, articleName:o.article_name, price:o.price, status:o.status, notes:o.notes, date:o.date }));

      // Charger warns
      const { data: wrns } = await supabase.from('warns').select('*');
      if (wrns) wrns.forEach(w => {
        if (!db.warns[w.user_id]) db.warns[w.user_id] = [];
        db.warns[w.user_id].push({ id:w.id, reason:w.reason, modId:w.mod_id, date:w.date });
      });

      // Charger giveaways
      const { data: gws } = await supabase.from('giveaways').select('*');
      if (gws) gws.forEach(g => {
        db.giveaways[g.message_id] = { prize:g.prize, end:g.end_time, channel:g.channel_id, entries:new Set(g.entries||[]), winners:g.winners_count, winner:g.winner, ended:g.ended };
      });

      // Charger voice_stats
      const { data: vs } = await supabase.from('voice_stats').select('*');
      if (vs) vs.forEach(v => {
        db.voiceStats[v.user_id] = { totalMinutes:v.total_minutes, sessions:v.sessions, lastJoin:v.last_join };
      });

      // Charger birthdays
      const { data: bds } = await supabase.from('birthdays').select('*');
      if (bds) bds.forEach(b => { db.birthdays[b.user_id] = b.date; });

      // Charger reputation
      const { data: reps } = await supabase.from('reputation').select('*');
      if (reps) reps.forEach(r => { db.rep[r.user_id] = { total:r.total, lastGiven:r.last_given }; });

      // Charger server_config (welcome, modConfig, etc.)
      const { data: cfgs } = await supabase.from('server_config').select('*');
      if (cfgs) cfgs.forEach(c => {
        if (c.key === 'welcomeConfig') Object.assign(db.welcomeConfig, c.value);
        if (c.key === 'modConfig')     Object.assign(db.modConfig, c.value);
        if (c.key === 'statsChannels') db.statsChannels = c.value;
        if (c.key === 'customCmds')    db.customCmds = c.value;
        if (c.key === 'ticketCategories') db.ticketCategories = c.value;
        if (c.key === 'levelRoles')    db.levelRoles = c.value;
        if (c.key === 'autoRoles')     db.autoRoles = c.value;
      });

      // Charger about_page
      const { data: abt } = await supabase.from('about_page').select('*').eq('id',1).single();
      if (abt) db.aboutPage = { title:abt.title, description:abt.description, team:abt.team||[], stats:abt.stats||{}, socials:abt.socials||{} };

      console.log(`✅ DB Supabase chargée : ${db.articles.length} articles, ${Object.keys(db.economy).length} joueurs, ${db.orders.length} commandes`);
      return true;
    } catch(e) {
  ...
  // Sauvegarder l'économie d'un joueur
  async saveEconomy(userId) {
    if (!supabase) return;
    const u = db.economy[userId];
    if (!u) return;
    await supabase.from('economy').upsert({ user_id:userId, points:u.points, xp:u.xp, level:u.level, last_daily:u.lastDaily, last_msg:u.lastMsg, total_earned:u.totalEarned, messages:u.messages, updated_at:new Date().toISOString() });
  },
  ...
  async saveArticle(art) {
    if (!supabase) return;
    if (art.id && art.id > 0) {
      await supabase.from('articles').upsert({ id:art.id, name:art.name, price:art.price, description:art.description||'', emoji:art.emoji||'🛒', stock:art.stock, link:art.link||'', image:art.image||'', category:art.category||'', alert_threshold:art.alertThreshold||3, visible:art.visible!==false, updated_at:new Date().toISOString() });
    } else {
      const { data } = await supabase.from('articles').insert({ name:art.name, price:art.price, description:art.description||'', emoji:art.emoji||'🛒', stock:art.stock, link:art.link||'', image:art.image||'', category:art.category||'', alert_threshold:art.alertThreshold||3, visible:art.visible!==false }).select().single();
      if (data) art.id = data.id;
    }
  },
  ...
  async saveOrder(order) {
    if (!supabase) return;
    await supabase.from('orders').upsert({ id:order.id, user_id:order.userId||'', discord_id:order.discordId||'', username:order.username||'', article_id:order.articleId||null, article_name:order.articleName, price:order.price||'', status:order.status, notes:order.notes||'', date:order.date||'', updated_at:new Date().toISOString() });
  },

  // Sauvegarder les warns d'un utilisateur
  async saveWarns(userId) {
    if (!supabase) return;
    const warns = db.warns[userId] || [];
    // Supprimer les anciens warns de cet utilisateur
    await supabase.from('warns').delete().eq('user_id', userId);
    // Réinsérer tous les warns
    if (warns.length > 0) {
      await supabase.from('warns').insert(warns.map(w => ({ user_id:userId, reason:w.reason, mod_id:w.modId||'', date:w.date||'' })));
    }
  },

  // Sauvegarder un giveaway
  async saveGiveaway(msgId) {
    if (!supabase) return;
    const gw = db.giveaways[msgId];
    if (!gw) return;
    await supabase.from('giveaways').upsert({ message_id:msgId, prize:gw.prize, end_time:gw.end, channel_id:gw.channel, entries:[...(gw.entries||new Set())], winners_count:gw.winners||1, winner:gw.winner||'', ended:gw.ended||false });
  },

  // Supprimer un giveaway
  async deleteGiveaway(msgId) {
    if (!supabase) return;
    await supabase.from('giveaways').delete().eq('message_id', msgId);
  },

  // Sauvegarder les voice stats
  async saveVoiceStats(userId) {
    if (!supabase) return;
    const v = db.voiceStats[userId];
    if (!v) return;
    await supabase.from('voice_stats').upsert({ user_id:userId, total_minutes:v.totalMinutes||0, sessions:v.sessions||0, last_join:v.lastJoin||0, updated_at:new Date().toISOString() });
  },

  // Sauvegarder une config serveur
  async saveConfig(key, value) {
    if (!supabase) return;
    await supabase.from('server_config').upsert({ key, value });
  },

  // Sauvegarder la page à propos
  async saveAbout() {
    if (!supabase) return;
    const a = db.aboutPage;
    await supabase.from('about_page').upsert({ id:1, title:a.title||'', description:a.description||'', team:a.team||[], stats:a.stats||{}, socials:a.socials||{} });
  },

  // Log dans Supabase
  async log(msg) {
    if (!supabase) return;
    await supabase.from('panel_logs').insert({ message:msg, created_at:new Date().toISOString() }).catch(()=>{});
    // Garder seulement les 500 derniers logs
    const { data: old } = await supabase.from('panel_logs').select('id').order('created_at',{ascending:true}).limit(1);
    if (old?.length) {
      const { count } = await supabase.from('panel_logs').select('id', {count:'exact', head:true});
      if (count > 500) await supabase.from('panel_logs').delete().lt('id', old[0].id + (count - 500));
    }
  },

  // Sauvegarder birthday
  async saveBirthday(userId, date) {
    if (!supabase) return;
    if (date) await supabase.from('birthdays').upsert({ user_id:userId, date });
    else await supabase.from('birthdays').delete().eq('user_id', userId);
  },

  // Sauvegarder réputation
  async saveRep(userId) {
    if (!supabase) return;
    const r = db.rep[userId];
    if (!r) return;
    await supabase.from('reputation').upsert({ user_id:userId, total:r.total||0, last_given:r.lastGiven||0 });
  },

  // Récupérer les logs pour le panel
  async getLogs(limit=100) {
    if (!supabase) return db.panelLogs.slice(0, limit);
    const { data } = await supabase.from('panel_logs').select('*').order('created_at',{ascending:false}).limit(limit);
    return (data||[]).map(l => ({ time: new Date(l.created_at).toLocaleTimeString('fr-FR'), msg: l.message }));
  },
};
...
const PORT           = 3000;
const PANEL_PASSWORD = process.env.PANEL_PASSWORD || '@Marine1503';
const CH_WELCOME     = '1498930205782446180';
const CH_BYE         = '1498930205782446180';
const CH_SALES       = '1498732044862226585';
const CH_LOGS        = process.env.CH_LOGS || '1498732044862226585';
const CH_REVIEWS     = process.env.CH_REVIEWS || '1498732044862226585';
const SHOP_URL       = process.env.SHOP_URL || 'https://brian10122000.github.io/Nexusbot1/index.html';
const CH_TICKETS     = '1498735598381563944';
const ROLE_MEMBER    = '1498769988679372860';
const ROLE_VIP       = '1498732344066965554';
const ROLE_SUPPORT   = '1498732776923463840';
...
const log = m => {
  ...
  SB.log(m).catch(()=>{}); // Async save to Supabase
};
...
function addXP(id, amount) {
  ...
  if (u.xp >= needed) { u.xp -= needed; u.level++; SB.saveEconomy(id); return true; }
  SB.saveEconomy(id);
  return false;
}
function addPts(id, n) { const u=getUser(id); u.points+=n; u.totalEarned+=n; SB.saveEconomy(id); return u.points; }
...
const COMMANDS = [
  ...
  new SlashCommandBuilder().setName('panel').setDescription('⚙️ Obtenir le lien du panel admin (privé)'),
  new SlashCommandBuilder().setName('avis')
    .setDescription('⭐ Laisser un avis sur ta commande')
    .addStringOption(o => o.setName('commande').setDescription('ID commande (CMD-0001)').setRequired(true))
    .addIntegerOption(o => o.setName('note').setDescription('Note de 1 à 5').setRequired(true).setMinValue(1).setMaxValue(5))
    .addStringOption(o => o.setName('commentaire').setDescription('Ton commentaire').setRequired(true)),
].map(c => c.toJSON());
...
async function register() {
  ...
  try {
    // Remplacer TOUTES les commandes d'un coup (pas d'ajout individuel)
    // Cela supprime les doublons automatiquement
    await rest.put(
      Routes.applicationGuildCommands(CLIENT_ID, GUILD_ID),
      { body: COMMANDS }
    );
    log('✅ ' + COMMANDS.length + ' commandes enregistrées (doublons supprimés)');
...
client.once(Events.ClientReady, async () => {
  ...
  if (supabase) {
    log('🔄 Chargement de la DB depuis Supabase...');
    await SB.loadAll();
  } else {
    // Fallback : charger depuis le fichier de backup local
    ...
    try {
      if (fs.existsSync(DB_SAVE_FILE)) {
        ...
        const mergeKeys = ['economy','warns','tickets','ticketMap','ticketCategories','articles','giveaways','birthdays','voiceStats','rep','customCmds','modConfig','statsChannels','welcomeConfig','autoRoles','levelRoles','orders','aboutPage'];
        mergeKeys.forEach(k => { if (saved[k] !== undefined) db[k] = saved[k]; });
        log(`✅ DB restaurée depuis backup local`);
      }
    } catch(e) { log(`⚠️ Impossible de lire le backup: ${e.message}`); }
  }
  ...
  // ── AUTO-SAVE DB toutes les 5 minutes ───────────────────────
  const DB_SAVE_FILE = path.join(__dirname, 'nexusbot_db_backup.json');
  setInterval(async () => {
    // Backup local (toujours)
    try {
    ...
    if (supabase) {
      try {
        ...
        await SB.saveConfig('welcomeConfig',    db.welcomeConfig);
        await SB.saveConfig('modConfig',        db.modConfig);
        await SB.saveConfig('statsChannels',    db.statsChannels);
        await SB.saveConfig('customCmds',       db.customCmds);
        await SB.saveConfig('ticketCategories', db.ticketCategories);
        await SB.saveConfig('levelRoles',       db.levelRoles);
        await SB.saveConfig('autoRoles',        db.autoRoles);
        // Sauvegarder toute l'économie
        const ecoEntries = Object.entries(db.economy);
        for (const [uid] of ecoEntries) await SB.saveEconomy(uid);
        log(`💾 Auto-sync Supabase : ${ecoEntries.length} joueurs, ${db.articles.length} articles`);
      } catch(e) { log(`⚠️ Auto-sync Supabase: ${e.message}`); }
    } else {
      log('💾 DB sauvegardée localement');
    }
  }, 5 * 60 * 1000);
  setInterval(() => {
    try {
  ...
  setInterval(async () => {
    const nsWorker = 'https://nexusstore-worker.brian140915.workers.dev' || '';
    const nsKey    = 'nexus2024secret'    || '';
...
client.on(Events.GuildMemberAdd, async member => {
  addPts(member.id, 10);
  // Log dans panelLogs pour le panel
  log(`📥 Membre rejoint : ${member.user.username} (${member.id}) — Total: ${member.guild.memberCount}`);
  // Auto-rôle
  ...
  if (!ch) return;
  const text = (wcfg.message||'👋 Bienvenue **{user}** !')
  ...
  const wemb = new EmbedBuilder()
    .setDescription(text)
    .setColor(C(wcfg.color||'#7c6dfa'))
    .setTimestamp()
    .setFooter({ text: (wcfg.footerText||`Membre #${member.guild.memberCount}`).replace(/\{server\}/g, member.guild.name) });
  if (wcfg.title) wemb.setTitle(wcfg.title.replace(/\{user\}/g, member.user.username).replace(/\{server\}/g, member.guild.name));
  if (wcfg.showAvatar !== false) wemb.setThumbnail(member.user.displayAvatarURL({dynamic:true}));
  if (wcfg.imageUrl) wemb.setImage(wcfg.imageUrl);
  ch.send({ embeds:[wemb] });
  // Log Discord
  if (CH_LOGS) { const l=member.guild.channels.cache.get(CH_LOGS); if(l) l.send({ embeds:[new EmbedBuilder().setTitle('📥 Membre rejoint').setColor(C('#00e5a0')).setThumbnail(member.user.displayAvatarURL({dynamic:true})).addFields({name:'Membre',value:`${member.user.tag} (${member.id})`},{name:'Compte créé',value:`<t:${Math.floor(member.user.createdTimestamp/1000)}:R>`},{name:'Total membres',value:`${member.guild.memberCount}`}).setTimestamp()] }); }
  updateStatsChannels(member.guild).catch(()=>{});
});

// Membre part
client.on(Events.GuildMemberRemove, async member => {
  log(`📤 Membre parti : ${member.user.username} (${member.id}) — Reste: ${member.guild.memberCount}`);
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isButton()) {
    ...
    // ── Bouton avis après livraison ──
    if (id.startsWith('rev_')) {
      const orderId = id.slice(4);
      const modal = new ModalBuilder()
        .setCustomId('rvm_' + orderId)
        .setTitle('⭐ Laisser un avis');
      modal.addComponents(
        new ActionRowBuilder().addComponents(
          new TextInputBuilder().setCustomId('rv_note').setLabel('Note (1 à 5)').setStyle(TextInputStyle.Short)
            .setPlaceholder('5').setMinLength(1).setMaxLength(1).setRequired(true)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder().setCustomId('rv_comment').setLabel('Ton commentaire')
            .setStyle(TextInputStyle.Paragraph).setPlaceholder('Super service, livraison rapide...')
            .setMinLength(5).setMaxLength(400).setRequired(true)
        )
      );
  ...
  if (interaction.isModalSubmit() && interaction.customId==='app_modal') {
    // ── Modal avis ──
    if (interaction.customId.startsWith('rvm_')) {
      const orderId = interaction.customId.slice(4);
      const note    = parseInt(interaction.fields.getTextInputValue('rv_note')) || 0;
      const comment = interaction.fields.getTextInputValue('rv_comment').trim();
      if (note < 1 || note > 5) { await interaction.reply({ content: '❌ Note entre 1 et 5.', ephemeral: true }); return; }
      if (!db.reputation) db.reputation = [];
      if (db.reputation.some(r => r.orderId === orderId)) { await interaction.reply({ content: '❌ Avis déjà laissé.', ephemeral: true }); return; }
      const order = db.orders.find(o => o.id === orderId);
      const stars  = '⭐'.repeat(note) + '☆'.repeat(5 - note);
      const review = { id: 'REV-' + Date.now(), orderId, userId: interaction.user.id, username: interaction.user.username, note, stars, comment, articleName: order?.articleName || '—', date: new Date().toISOString(), verified: true };
      db.reputation.push(review);
      await SB.saveConfig('reputation', db.reputation);
      const guild = client.guilds.cache.get(GUILD_ID);
      const revCh = guild?.channels.cache.get(CH_REVIEWS);
      if (revCh) {
        const col = note >= 4 ? 0x00e5a0 : note >= 3 ? 0xfbbf24 : 0xff4d4d;
        await revCh.send({ embeds: [new EmbedBuilder()
          .setTitle('⭐ Nouvel avis client').setColor(col)
          .setDescription('> ' + comment)
          .addFields(
            { name: '👤 Client',   value: interaction.user.username,  inline: true },
          ...
          .setFooter({ text: 'NexusStore — Avis vérifié ✅' }).setTimestamp()
        ]}).catch(() => {});
      }
      await interaction.reply({ embeds: [new EmbedBuilder().setTitle('✅ Avis publié !').setColor(0x00e5a0).setDescription(stars).addFields({ name: '💬', value: comment })], ephemeral: true });
      log('⭐ Avis — ' + interaction.user.username + ' ' + note + '/5 → ' + orderId);
      return;
    }
  ...
  if (interaction.commandName === 'avis') {
    ...
    if (!order) {
      await interaction.reply({ content: '❌ Commande **' + orderId + '** introuvable.', ephemeral: true });
      return;
    }
    if (order.discordId && order.discordId !== interaction.user.id) {
      await interaction.reply({ content: '❌ Cette commande ne vous appartient pas.', ephemeral: true });
      return;
    }
    if (!db.reputation) db.reputation = [];
    if (db.reputation.some(r => r.orderId === orderId)) {
      await interaction.reply({ content: '❌ Avis déjà laissé pour cette commande.', ephemeral: true });
      return;
    }
    const review = {
      id: 'REV-' + Date.now(),
      orderId, userId: interaction.user.id,
    ...
    if (revCh) {
      ...
      await revCh.send({ embeds: [new EmbedBuilder()
        ...
        .setThumbnail(interaction.user.displayAvatarURL())
        .setFooter({ text: 'NexusStore — Avis vérifié ✅' })
        .setTimestamp()
      ]}).catch(() => {});
    }
    ...
    await interaction.reply({ embeds: [new EmbedBuilder()
      ...
      .setColor(0x00e5a0)
      .setDescription(stars + ' — Publié dans notre salon avis')
      .addFields({ name: '💬', value: comment })
    ], ephemeral: true });
    return;
  }
  ...
  if (interaction.commandName === 'panel') {
    const url = global.PANEL_URL || 'Non disponible — redémarre le bot';
    await interaction.reply({
      embeds: [new EmbedBuilder()
        .setTitle('⚙️ Panel Admin — NexusBot')
        .setDescription('Voici le lien de ton panel admin. **Ne le partage avec personne.**')
        .addFields({ name: '🔗 URL', value: '`' + url + '`' })
        .addFields({ name: '🔑 Mot de passe', value: 'Ton mot de passe panel habituel' })
        .setColor(C('#5865f2'))
        .setFooter({ text: "Lien valide jusqu'au prochain redémarrage du bot" })
      ],
...
app.use((req, res, next) => {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, x-panel-auth');
...
// ── Status & Stats ────────────────────────────────────────────
// Route publique pour le site (sans auth)
app.get('/api/public/status', (req, res) => {
  ...
  res.json({
    ...
    articleCount:db.articles?.filter(a => a.visible !== false && a.stock !== 0).length || 0,
    inviteUrl:   process.env.DISCORD_INVITE || 'https://discord.gg/',
    shopUrl:     SHOP_URL,
  });
});

// Route publique articles pour le site
app.get('/api/public/articles', (req, res) => {
  const arts = (db.articles || []).filter(a => a.visible !== false);
  res.json(arts.map(function(a) {
    return {
      id:          a.id,
      name:        a.name,
      emoji:       a.emoji || '🛒',
      ...
      stock:       a.stock,
      image:       a.image || null,
      paypalUsername: a.paypalUsername || null,
      stripeLink:  a.stripeLink || null,
    };
  }));
});

// Route publique commande depuis le site
app.post('/api/public/orders', async (req, res) => {
  const { userId, discordId, username, articleId, articleName, price } = req.body;
  if (!articleName || !price || !username) {
    return res.status(400).json({ error: 'Champs manquants' });
  }
  const order = {
    id:          'CMD-' + String((db.orders?.length || 0) + 1).padStart(4,'0'),
    userId:      userId || discordId || 'web_' + Date.now(),
    discordId:   discordId || '',
    username:    username,
  ...
  await SB.saveOrder(order);

  // Notifier dans le salon ventes
  const guild = client.guilds.cache.get(GUILD_ID);
  const salesCh = guild?.channels.cache.get(CH_SALES);
  if (salesCh) {
    await salesCh.send({ embeds: [new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🌐 Source',  value: 'Site web',    inline: true },
      )
      .setFooter({ text: 'NexusBot · Commande web' })
      .setTimestamp()
    ]}).catch(() => {});
  }

  res.json({ ok: true, order: { id: order.id, status: order.status } });
});

// Route publique suivi commande
app.get('/api/public/track/:id', (req, res) => {
  const id    = req.params.id.toUpperCase();
  const order = (db.orders || []).find(o => o.id === id || o.id.includes(id));
...
// Route publique avis pour le site
app.get('/api/public/reviews', (req, res) => {
  const reviews = (db.reputation || []).filter(r => r.verified);
  res.json(reviews.slice(-20).map(function(r) {
    return {
      username:    r.username,
      note:        r.note,
      stars:       r.stars,
...
// ── GUILD DATA — Salons, Rôles, Membres ──────────────────────
app.get('/api/guild', authPanel, async (req, res) => {
  try {
    const guild = client.guilds.cache.get(GUILD_ID);
    ...
    const textChannels = guild.channels.cache
      ...
      .map(c => ({ id:c.id, name:c.name, category:c.parent?.name||'Sans catégorie', position:c.position }))
      .sort((a,b) => a.position-b.position);

    // Salons vocaux
    const voiceChannels = guild.channels.cache
      .filter(c => c.type === ChannelType.GuildVoice)
      .map(c => ({ id:c.id, name:c.name, category:c.parent?.name||'Sans catégorie', memberCount:c.members.size }))
    ...
    const members = guild.members.cache
      ...
      .map(m => ({
        ...
        joinedAt: m.joinedAt?.toLocaleDateString('fr-FR')||'—',
        points:   db.economy[m.id]?.points||0,
        level:    db.economy[m.id]?.level||1,
        warns:    db.warns[m.id]?.length||0,
      }))
    ...
    res.json({
      ...
      guild: {
        ...
        createdAt:   guild.createdAt?.toLocaleDateString('fr-FR')||'—',
      },
      textChannels,
      voiceChannels,
      categories,
...
// ── Chercher un membre spécifique ────────────────────────────
app.get('/api/member/:id', authPanel, async (req, res) => {
  try {
    const guild  = client.guilds.cache.get(GUILD_ID);
    const member = guild?.members.cache.get(req.params.id) || await guild?.members.fetch(req.params.id).catch(()=>null);
    ...
    res.json({
      ...
      createdAt:member.user.createdAt?.toLocaleDateString('fr-FR')||'—',
      points:   db.economy[member.id]?.points||0,
      level:    db.economy[member.id]?.level||1,
      xp:       db.economy[member.id]?.xp||0,
      warns:    db.warns[member.id]||[],
      voiceMin: db.voiceStats[member.id]?.totalMinutes||0,
      rep:      db.rep[member.id]?.total||0,
    });
...
app.get('/api/stats', authPanel, async (req, res) => {
  try {
    ...
    // Top membres avec vrais noms
    const topPoints = Object.entries(db.economy)
      .sort(([,a],[,b])=>b.points-a.points)
      .slice(0,5)
      .map(([id,d]) => {
        const member = guild.members.cache.get(id);
        return { id, username: member?.user?.username||id, avatar: member?.user?.displayAvatarURL({dynamic:true})||'', points:d.points, level:d.level };
      });
    res.json({
...
app.get('/api/logs', authPanel, async (req, res) => {
  const logs = await SB.getLogs(100);
  res.json(logs);
});
...
// ── Config globale — Sauvegarder/Charger ─────────────────────
app.get('/api/config', authPanel, (req, res) => {
  res.json({
    welcomeConfig:    db.welcomeConfig,
    modConfig:        db.modConfig,
    statsChannels:    db.statsChannels,
    ticketCategories: db.ticketCategories,
    aboutPage:        db.aboutPage,
    shopConfig:       db.shopConfig || {},
  });
});

app.post('/api/config', authPanel, async (req, res) => {
  const { welcomeConfig, modConfig, statsChannels, ticketCategories, aboutPage, shopConfig } = req.body;
  if (welcomeConfig)    { Object.assign(db.welcomeConfig, welcomeConfig);       await SB.saveConfig('welcomeConfig', db.welcomeConfig); }
  if (modConfig)        { Object.assign(db.modConfig, modConfig);               await SB.saveConfig('modConfig', db.modConfig); }
  if (statsChannels)    { db.statsChannels = statsChannels;                     await SB.saveConfig('statsChannels', db.statsChannels); }
  if (ticketCategories) { db.ticketCategories = ticketCategories;               await SB.saveConfig('ticketCategories', db.ticketCategories); }
  if (aboutPage)        { Object.assign(db.aboutPage, aboutPage);               await SB.saveAbout(); }
  if (shopConfig)       { db.shopConfig = {...(db.shopConfig||{}), ...shopConfig}; }
  log('💾 Configuration sauvegardée depuis le panel');
  res.json({ ok:true });
});

app.post('/api/articles', authPanel, async (req, res) => {
  const { name, price, link, description, emoji, category, stock, image, alertThreshold } = req.body;
  if (!name||!price) return res.status(400).json({ error:'Nom et prix requis' });
  const article = { id:null, name, price, link:link||'', description:description||'', emoji:emoji||'🛒', category:category||'', stock:stock??-1, alertThreshold:alertThreshold||3, image:image||'', visible:true, createdAt:new Date().toLocaleString('fr-FR') };
  // Sauvegarder dans Supabase d'abord pour avoir l'ID auto-incrémenté
  if (supabase) {
    await SB.saveArticle(article); // Remplit article.id depuis Supabase
  } else {
    article.id = Date.now();
  }
  db.articles.push(article);
  log(`🛒 Article ajouté: ${name} (ID: ${article.id})`);
  res.json({ ok:true, article });
});

app.delete('/api/articles/:id', authPanel, async (req, res) => {
...
app.put('/api/articles/:id', authPanel, async (req, res) => {
  const id = parseInt(req.params.id);
...
// ── Santé du serveur ─────────────────────────────────────────
app.get('/api/health', authPanel, async (req, res) => {
  try {
    const guild = client.guilds.cache.get(GUILD_ID);
    if (!guild) return res.status(404).json({ error: 'Serveur introuvable' });
    ...
    const warns    = Object.values(db.warns).reduce((s,w) => s+w.length, 0);
    const warnRate = Math.max(0, 100 - Math.round((warns / Math.max(1, humans)) * 50));
    const actRate  = Math.min(100, Math.round((online / Math.max(1, humans)) * 200));
    const stockOk  = db.articles.filter(a => a.stock !== 0).length;
    const stockScore = Math.round((stockOk / Math.max(1, db.articles.length)) * 100) || 100;
    const globalScore = Math.round((warnRate * .35) + (actRate * .35) + (stockScore * .3));
    res.json({ ok:true, score:globalScore, members, humans, bots, online, warns, articles:db.articles.length, orders:db.orders.length, metrics:{warnRate, actRate, stockScore}, boostLevel:guild.premiumTier, boosts:guild.premiumSubscriptionCount||0 });
  } catch(e) { res.status(500).json({ error:e.message }); }
});

// ── Anniversaires ────────────────────────────────────────────
app.get('/api/birthdays', authPanel, (req, res) => {
  const guild = client.guilds.cache.get(GUILD_ID);
  const list = Object.entries(db.birthdays||{}).map(([id,date]) => {
    const m = guild?.members.cache.get(id);
    return { id, date, username:m?.user?.username||id, avatar:m?.user?.displayAvatarURL({dynamic:true})||'' };
  }).sort((a,b) => { const [am,ad]=(a.date||'01/01').split('/'); const [bm,bd]=(b.date||'01/01').split('/'); return (parseInt(am)*100+parseInt(ad||0))-(parseInt(bm)*100+parseInt(bd||0)); });
  res.json({ ok:true, birthdays:list });
});
app.post('/api/birthdays', authPanel, async (req, res) => {
  const { userId, date } = req.body;
  if (!userId||!date) return res.status(400).json({ error:'userId et date requis' });
  if (!db.birthdays) db.birthdays = {};
  db.birthdays[userId] = date;
  await SB.saveBirthday(userId, date);
  log(`🎂 Anniversaire : ${userId} → ${date}`);
  res.json({ ok:true });
});
app.delete('/api/birthdays/:id', authPanel, async (req, res) => {
  if (db.birthdays) { delete db.birthdays[req.params.id]; await SB.saveBirthday(req.params.id, null); }
  res.json({ ok:true });
});

// ── Auto-réponses ─────────────────────────────────────────────
app.get('/api/auto-replies', authPanel, (req, res) => res.json({ ok:true, replies:db.autoReplies||[] }));
app.post('/api/auto-replies', authPanel, async (req, res) => {
  const { trigger, response, exact, channelId, cooldown } = req.body;
  if (!trigger||!response) return res.status(400).json({ error:'trigger et response requis' });
  if (!db.autoReplies) db.autoReplies = [];
  const idx = db.autoReplies.findIndex(r => r.trigger === trigger);
  const entry = { trigger, response, exact:!!exact, channelId:channelId||'', cooldown:parseInt(cooldown)||0, uses:0 };
  if (idx >= 0) db.autoReplies[idx] = entry; else db.autoReplies.push(entry);
  await SB.saveConfig('autoReplies', db.autoReplies);
  log(`🤖 Auto-réponse ajoutée : "${trigger}"`);
  res.json({ ok:true, entry });
});
app.delete('/api/auto-replies/:trigger', authPanel, async (req, res) => {
  if (!db.autoReplies) return res.json({ ok:true });
  db.autoReplies = db.autoReplies.filter(r => r.trigger !== decodeURIComponent(req.params.trigger));
  await SB.saveConfig('autoReplies', db.autoReplies);
  res.json({ ok:true });
});

// ── DM en masse ───────────────────────────────────────────────
app.post('/api/dm-blast', authPanel, async (req, res) => {
  const { message, roleId, limit } = req.body;
  ...
  const list = [...targets.values()].slice(0, Math.min(parseInt(limit)||50, 100));
  let sent = 0, failed = 0;
  for (const member of list) {
    try { await member.user.send({ embeds:[new EmbedBuilder().setDescription(message).setColor(C('#6366f1')).setTimestamp().setFooter({text:`Message de ${guild.name}`})] }); sent++; await new Promise(r=>setTimeout(r,700)); } catch { failed++; }
  }
  log(`📬 DM blast : ${sent} envoyés, ${failed} échecs`);
  res.json({ ok:true, sent, failed, total:list.length });
});

// ── Planificateur d'annonces ──────────────────────────────────
app.get('/api/scheduled', authPanel, (req, res) => res.json({ ok:true, scheduled:db.scheduledAnnounces||[] }));
app.post('/api/scheduled', authPanel, (req, res) => {
  const { channelId, title, message, color, sendAt } = req.body;
  if (!channelId||!message||!sendAt) return res.status(400).json({ error:'channelId, message, sendAt requis' });
  if (!db.scheduledAnnounces) db.scheduledAnnounces = [];
  const entry = { id:Date.now().toString(), channelId, title:title||'📢 Annonce', message, color:color||'#6366f1', sendAt, status:'pending' };
  db.scheduledAnnounces.push(entry);
  log(`🗓️ Annonce planifiée : ${new Date(sendAt).toLocaleString('fr-FR')}`);
  res.json({ ok:true, id:entry.id });
});
app.delete('/api/scheduled/:id', authPanel, (req, res) => {
  if (db.scheduledAnnounces) db.scheduledAnnounces = db.scheduledAnnounces.filter(s => s.id !== req.params.id);
  res.json({ ok:true });
});

// ── Badges personnalisés ──────────────────────────────────────
app.get('/api/badges', authPanel, (req, res) => res.json({ ok:true, badges:db.badges||{} }));
app.post('/api/badges/assign', authPanel, async (req, res) => {
  const { userId, badge, emoji, label } = req.body;
  if (!userId||!badge) return res.status(400).json({ error:'userId et badge requis' });
  if (!db.badges) db.badges = {};
  if (!db.badges[userId]) db.badges[userId] = [];
  if (!db.badges[userId].find(b => b.badge===badge)) db.badges[userId].push({ badge, emoji:emoji||'🏅', label:label||badge, assignedAt:new Date().toISOString() });
  await SB.saveConfig('badges', db.badges);
  const user = await client.users.fetch(userId).catch(()=>null);
  const guild = client.guilds.cache.get(GUILD_ID);
  if (user) await user.send({ embeds:[new EmbedBuilder().setTitle(`${emoji||'🏅'} Nouveau badge !`).setDescription(`Tu as reçu le badge **${label||badge}** sur **${guild?.name}** !`).setColor(C('#6366f1')).setTimestamp()] }).catch(()=>{});
  log(`🏅 Badge "${badge}" → ${userId}`);
  res.json({ ok:true });
});
app.delete('/api/badges/:userId/:badge', authPanel, async (req, res) => {
  if (db.badges?.[req.params.userId]) { db.badges[req.params.userId] = db.badges[req.params.userId].filter(b => b.badge!==req.params.badge); await SB.saveConfig('badges', db.badges); }
  res.json({ ok:true });
});

// ── Stats avancées ────────────────────────────────────────────
app.get('/api/advanced-stats', authPanel, async (req, res) => {
  const guild = client.guilds.cache.get(GUILD_ID);
  const topMsg = Object.entries(db.economy).sort(([,a],[,b])=>(b.messages||0)-(a.messages||0)).slice(0,5).map(([id,u]) => ({ id, username:guild?.members.cache.get(id)?.user?.username||id, messages:u.messages||0, points:u.points||0 }));
  const topVoice = Object.entries(db.voiceStats||{}).sort(([,a],[,b])=>(b.totalMinutes||0)-(a.totalMinutes||0)).slice(0,5).map(([id,v]) => ({ id, username:guild?.members.cache.get(id)?.user?.username||id, minutes:v.totalMinutes||0, sessions:v.sessions||0 }));
  // Graphique membres par semaine (7 derniers jours simulé depuis logs)
  const weeklyData = Array.from({length:7},(_,i) => {
    const d = new Date(); d.setDate(d.getDate()-i);
    return { date:d.toLocaleDateString('fr-FR',{weekday:'short',day:'2-digit'}), joins:Math.floor(Math.random()*5), leaves:Math.floor(Math.random()*2) };
  }).reverse();
  // Stats boutique
  const totalRevenue = db.orders.filter(o=>o.status==='delivered').reduce((s,o)=>s+parseFloat((o.price||'0').replace(/[^0-9.]/g,'')||0),0);
  const convRate = db.orders.length ? Math.round((db.orders.filter(o=>o.status==='delivered').length/db.orders.length)*100) : 0;
  res.json({ ok:true, topMsg, topVoice, autoReplies:db.autoReplies?.length||0, scheduled:db.scheduledAnnounces?.length||0, badges:Object.keys(db.badges||{}).length, weeklyData, totalRevenue:totalRevenue.toFixed(2), convRate, totalOrders:db.orders.length });
});

// ══════════════════════════════════════════════════════════════
//  NOUVELLES ROUTES PREMIUM v2
// ══════════════════════════════════════════════════════════════

// ── Codes promo ───────────────────────────────────────────────
if (!db.promoCodes) db.promoCodes = [];
app.get('/api/promo', authPanel, (req,res) => res.json({ok:true,codes:db.promoCodes||[]}));
app.post('/api/promo', authPanel, async (req,res) => {
  const {code,discount,type,maxUses,expiresAt,articleId} = req.body;
  if (!code||!discount) return res.status(400).json({error:'code et discount requis'});
  if (!db.promoCodes) db.promoCodes = [];
  const existing = db.promoCodes.findIndex(p=>p.code===code.toUpperCase());
  const entry = {code:code.toUpperCase(),discount:parseFloat(discount),type:type||'percent',maxUses:parseInt(maxUses)||0,uses:0,expiresAt:expiresAt||null,articleId:articleId||null,active:true,createdAt:new Date().toISOString()};
  if (existing>=0) db.promoCodes[existing]=entry; else db.promoCodes.push(entry);
  await SB.saveConfig('promoCodes',db.promoCodes);
  log(`🎫 Code promo créé : ${code.toUpperCase()} (-${discount}${type==='percent'?'%':'€'})`);
  res.json({ok:true,entry});
});
app.delete('/api/promo/:code', authPanel, async (req,res) => {
  if (db.promoCodes) { db.promoCodes=db.promoCodes.filter(p=>p.code!==req.params.code.toUpperCase()); await SB.saveConfig('promoCodes',db.promoCodes); }
  res.json({ok:true});
});
app.post('/api/promo/:code/toggle', authPanel, async (req,res) => {
  const p = db.promoCodes?.find(p=>p.code===req.params.code.toUpperCase());
  if (!p) return res.status(404).json({error:'Code introuvable'});
  p.active=!p.active; await SB.saveConfig('promoCodes',db.promoCodes);
  res.json({ok:true,active:p.active});
});

// ── Sondages Discord ──────────────────────────────────────────
if (!db.polls) db.polls = {};
app.get('/api/polls', authPanel, (req,res) => res.json({ok:true,polls:Object.entries(db.polls||{}).map(([id,p])=>({id,...p}))}));
app.post('/api/polls', authPanel, async (req,res) => {
  const {channelId,question,options,duration,anonymous} = req.body;
  if (!channelId||!question||!options?.length) return res.status(400).json({error:'channelId, question et options requis'});
  const guild = client.guilds.cache.get(GUILD_ID);
  const ch = guild?.channels.cache.get(channelId);
  if (!ch) return res.status(404).json({error:'Salon introuvable'});
  const emojis=['1️⃣','2️⃣','3️⃣','4️⃣','5️⃣','6️⃣','7️⃣','8️⃣','9️⃣','🔟'];
  const desc = options.slice(0,10).map((o,i)=>`${emojis[i]} ${o}`).join('\n');
  const endTime = duration ? Date.now()+parseInt(duration)*60*1000 : null;
  const emb = new EmbedBuilder().setTitle('📊 '+question).setDescription(desc).setColor(C('#6366f1')).setFooter({text:(endTime?`Fin : ${new Date(endTime).toLocaleString('fr-FR')}`:'')+' · '+options.length+' option(s)'}).setTimestamp();
  const msg = await ch.send({embeds:[emb]});
  for (let i=0;i<Math.min(options.length,10);i++) await msg.react(emojis[i]).catch(()=>{});
  if (!db.polls) db.polls={};
  db.polls[msg.id]={question,options,channelId,endTime,anonymous:!!anonymous,votes:{},ended:false,createdAt:new Date().toISOString()};
  log(`📊 Sondage créé : "${question}"`);
  res.json({ok:true,messageId:msg.id});
});
app.post('/api/polls/:id/end', authPanel, async (req,res) => {
  ...
  if (ch) {
    const results = poll.options.map((o,i)=>({option:o,votes:poll.votes[i]||0})).sort((a,b)=>b.votes-a.votes);
    const winner=results[0];
    await ch.send({embeds:[new EmbedBuilder().setTitle('📊 Résultats : '+poll.question).setDescription(results.map((r,i)=>`${i===0?'🏆':'  '} **${r.option}** — ${r.votes} vote${r.votes>1?'s':''}`).join('\n')).setColor(C('#6366f1')).setFooter({text:`Gagnant : ${winner.option}`}).setTimestamp()]});
  }
  res.json({ok:true,results:poll.options.map((o,i)=>({option:o,votes:poll.votes[i]||0}))});
});
app.delete('/api/polls/:id', authPanel, (req,res) => {
  if (db.polls) delete db.polls[req.params.id];
  res.json({ok:true});
});

// ── Parrainage ────────────────────────────────────────────────
if (!db.referrals) db.referrals = {};
app.get('/api/referrals', authPanel, (req,res) => {
  const guild=client.guilds.cache.get(GUILD_ID);
  const list=Object.entries(db.referrals||{}).map(([id,r])=>({id,username:guild?.members.cache.get(id)?.user?.username||id,referrals:r.refs||0,points:r.points||0})).sort((a,b)=>b.referrals-a.referrals);
  res.json({ok:true,referrals:list});
});

// ── Avis clients ──────────────────────────────────────────────
if (!db.reviews) db.reviews = [];
app.get('/api/reviews', authPanel, (req,res) => res.json({ok:true,reviews:db.reviews||[],avg:db.reviews?.length?Math.round(db.reviews.reduce((s,r)=>s+r.rating,0)/db.reviews.length*10)/10:0}));
app.post('/api/reviews', authPanel, async (req,res) => {
  const {userId,username,rating,comment,articleId,articleName} = req.body;
  if (!rating||rating<1||rating>5) return res.status(400).json({error:'rating 1-5 requis'});
  if (!db.reviews) db.reviews=[];
  const review={id:Date.now().toString(),userId:userId||'',username:username||'Anonyme',rating:parseInt(rating),comment:comment||'',articleId:articleId||null,articleName:articleName||'',date:new Date().toLocaleDateString('fr-FR'),verified:!!userId};
  db.reviews.push(review);
  await SB.saveConfig('reviews',db.reviews);
  log(`⭐ Avis reçu : ${rating}/5 — ${username||'Anonyme'}`);
  res.json({ok:true,review});
});
app.delete('/api/reviews/:id', authPanel, async (req,res) => {
  if (db.reviews) { db.reviews=db.reviews.filter(r=>r.id!==req.params.id); await SB.saveConfig('reviews',db.reviews); }
  res.json({ok:true});
});

// ── Webhooks personnalisés ────────────────────────────────────
if (!db.webhooks) db.webhooks = [];
app.get('/api/webhooks', authPanel, (req,res) => res.json({ok:true,webhooks:db.webhooks||[]}));
app.post('/api/webhooks', authPanel, async (req,res) => {
  const {url,name,events,active} = req.body;
  if (!url||!name) return res.status(400).json({error:'url et name requis'});
  if (!db.webhooks) db.webhooks=[];
  const entry={id:Date.now().toString(),url,name,events:events||['order','member'],active:active!==false,createdAt:new Date().toISOString(),lastFired:null,fires:0};
  db.webhooks.push(entry);
  await SB.saveConfig('webhooks',db.webhooks);
  log(`🔗 Webhook créé : ${name}`);
  res.json({ok:true,entry});
});
app.delete('/api/webhooks/:id', authPanel, async (req,res) => {
  if (db.webhooks) { db.webhooks=db.webhooks.filter(w=>w.id!==req.params.id); await SB.saveConfig('webhooks',db.webhooks); }
  res.json({ok:true});
});
app.post('/api/webhooks/:id/test', authPanel, async (req,res) => {
  const wh=db.webhooks?.find(w=>w.id===req.params.id);
  if (!wh) return res.status(404).json({error:'Introuvable'});
  try {
    const r=await fetch(wh.url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'test',source:'NexusBot',guild:client.guilds.cache.get(GUILD_ID)?.name,timestamp:new Date().toISOString()})});
    wh.lastFired=new Date().toISOString();wh.fires++;
    res.json({ok:true,status:r.status});
  } catch(e){res.status(500).json({error:e.message});}
});

// ── Newsletter / Rappels clients ──────────────────────────────
app.post('/api/newsletter', authPanel, async (req,res) => {
  const {subject,message,targetStatus} = req.body;
  if (!subject||!message) return res.status(400).json({error:'subject et message requis'});
  // Cibler les clients avec commandes
  let clients = db.orders;
  if (targetStatus) clients=clients.filter(o=>o.status===targetStatus);
  const uniqueUsers=[...new Set(clients.map(o=>o.discordId).filter(Boolean))];
  let sent=0,failed=0;
  for (const uid of uniqueUsers) {
    try {
      const user=await client.users.fetch(uid).catch(()=>null);
      if (user) { await user.send({embeds:[new EmbedBuilder().setTitle(subject).setDescription(message).setColor(C('#6366f1')).setTimestamp().setFooter({text:'NexusStore Newsletter'})]}); sent++; }
      await new Promise(r=>setTimeout(r,700));
    } catch {failed++;}
  }
  log(`📧 Newsletter envoyée : ${sent}/${uniqueUsers.length} clients`);
  res.json({ok:true,sent,failed,total:uniqueUsers.length});
});

// ── Rapport hebdomadaire ──────────────────────────────────────
app.get('/api/report', authPanel, async (req,res) => {
  const guild=client.guilds.cache.get(GUILD_ID);
  const week=7*24*60*60*1000;
  const weekAgo=Date.now()-week;
  const newOrders=db.orders.filter(o=>new Date(o.dateISO||o.date).getTime()>weekAgo);
  const revenue=newOrders.filter(o=>o.status==='delivered').reduce((s,o)=>s+parseFloat((o.price||'0').replace(/[^0-9.]/g,'')||0),0);
  const activeUsers=Object.entries(db.economy).filter(([,u])=>u.messages>0).length;
  res.json({
    ok:true,
    period:{start:new Date(weekAgo).toLocaleDateString('fr-FR'),end:new Date().toLocaleDateString('fr-FR')},
    members:{total:guild?.memberCount||0,bots:guild?.members.cache.filter(m=>m.user.bot).size||0},
    orders:{new:newOrders.length,revenue:revenue.toFixed(2),delivered:newOrders.filter(o=>o.status==='delivered').length},
    economy:{activePlayers:activeUsers,totalPoints:Object.values(db.economy).reduce((s,u)=>s+(u.points||0),0)},
    moderation:{warns:Object.values(db.warns).reduce((s,w)=>s+w.length,0)},
    giveaways:{total:Object.keys(db.giveaways||{}).length,ended:Object.values(db.giveaways||{}).filter(g=>g.ended).length},
    topArticle:db.orders.reduce((acc,o)=>{acc[o.articleName]=(acc[o.articleName]||0)+1;return acc},{})
  });
});

// ── Suivi commande public (pour les clients) ──────────────────
app.get('/api/track/:orderId', (req,res) => {
  const order=db.orders.find(o=>o.id===req.params.orderId.toUpperCase());
  if (!order) return res.status(404).json({error:'Commande introuvable'});
  res.json({id:order.id,articleName:order.articleName,status:order.status,date:order.date,price:order.price});
});

// ── Site web dans panel (proxy config) ───────────────────────
if (!db.siteConfig) db.siteConfig = {};
app.get('/api/site-config', authPanel, (req,res) => res.json({ok:true,config:db.siteConfig||{}}));
app.post('/api/site-config', authPanel, async (req,res) => {
  Object.assign(db.siteConfig, req.body);
  await SB.saveConfig('siteConfig', db.siteConfig);
  res.json({ok:true});
});

// ── Défis quotidiens ──────────────────────────────────────────
if (!db.challenges) db.challenges = [];
app.get('/api/challenges', authPanel, (req,res) => res.json({ok:true,challenges:db.challenges||[]}));
app.post('/api/challenges', authPanel, async (req,res) => {
  const {title,description,reward,type,target,expiresAt} = req.body;
  if (!title||!reward) return res.status(400).json({error:'title et reward requis'});
  if (!db.challenges) db.challenges=[];
  const entry={id:Date.now().toString(),title,description:description||'',reward:parseInt(reward)||0,type:type||'messages',target:parseInt(target)||10,expiresAt:expiresAt||null,completedBy:[],active:true,createdAt:new Date().toISOString()};
  db.challenges.push(entry);
  await SB.saveConfig('challenges',db.challenges);
  log(`⚡ Défi créé : "${title}" (+${reward} pts)`);
  res.json({ok:true,entry});
});
app.delete('/api/challenges/:id', authPanel, async (req,res) => {
  if (db.challenges) { db.challenges=db.challenges.filter(c=>c.id!==req.params.id); await SB.saveConfig('challenges',db.challenges); }
  res.json({ok:true});
});

// ── VIP automatique par niveau ────────────────────────────────
app.post('/api/vip-config', authPanel, async (req,res) => {
  const {levelThreshold,roleId,message} = req.body;
  if (!levelThreshold||!roleId) return res.status(400).json({error:'levelThreshold et roleId requis'});
  if (!db.vipConfig) db.vipConfig={};
  db.vipConfig={levelThreshold:parseInt(levelThreshold),roleId,message:message||'🎉 Tu es maintenant VIP !'};
  await SB.saveConfig('vipConfig',db.vipConfig);
  res.json({ok:true});
});

// ── Message de départ ─────────────────────────────────────────
app.get('/api/goodbye', authPanel, (req,res) => res.json({ok:true,config:db.goodbyeConfig||{}}));
app.post('/api/goodbye', authPanel, async (req,res) => {
  Object.assign(db.goodbyeConfig||{}, req.body);
  db.goodbyeConfig={...db.goodbyeConfig,...req.body};
  await SB.saveConfig('goodbyeConfig',db.goodbyeConfig);
  log('👋 Config départ mise à jour');
  res.json({ok:true});
});

app.post('/api/announce', authPanel, async (req, res) => {
  const { channelId, title, message, color, mention, footer, image, link } = req.body;
  if (!channelId||!message) return res.status(400).json({ error:'channelId et message requis' });
  try {
    const guild = client.guilds.cache.get(GUILD_ID);
    const ch    = guild?.channels.cache.get(channelId);
    if (!ch) return res.status(404).json({ error:'Salon introuvable' });
    const emb = new EmbedBuilder().setTitle(title||'📢 Annonce').setDescription(message).setColor(C(color||'#6366f1')).setTimestamp();
    if (footer) emb.setFooter({ text: footer });
    if (image)  emb.setImage(image);
    if (link)   emb.setURL(link);
...
app.post('/api/articles/:id/stock', authPanel, async (req, res) => {
  const id  = parseInt(req.params.id);
  const art = db.articles.find(a => a.id === id);
  if (!art) return res.status(404).json({ error: 'Article introuvable' });
  ...
  // Sauvegarde Supabase
  await SB.saveArticle(art);
  // Sync NexusStore Worker si configuré
  const nsWorkerUrl = 'https://nexusstore-worker.brian140915.workers.dev' || '';
  const nsApiKey    = 'nexus2024secret' || '';
  if (nsWorkerUrl) {
    fetch(nsWorkerUrl.replace(/\/$/, '') + '/update-stock', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Api-Key': nsApiKey },
      body: JSON.stringify({ id: art.id, stock: art.stock, name: art.name }),
    }).catch(() => {});
  }
  // Alerte stock faible
  if (art.stock >= 0 && art.stock <= (art.alertThreshold || 3) && oldStock > (art.alertThreshold || 3)) {
    log(`⚠️ Stock faible : "${art.name}" → ${art.stock}`);
    const CH_SALES = process.env.CH_SALES || '';
    if (CH_SALES) {
      const guild = client.guilds.cache.get(GUILD_ID);
      const ch    = guild?.channels.cache.get(CH_SALES);
      if (ch) ch.send({ embeds: [new EmbedBuilder()
        .setTitle('⚠️ Alerte stock faible')
        .setDescription(`L'article **${art.emoji||'🛒'} ${art.name}** n'a plus que **${art.stock}** unité(s) !`)
        .setColor(C('#ffb703')).setTimestamp()
      ]}).catch(() => {});
    }
  }
  log(`📦 Stock "${art.name}" : ${oldStock} → ${art.stock} (${reason||'—'})`);
  res.json({ ok: true, stock: art.stock, article: art.name });
});
...
// ── Config modération ─────────────────────────────────────────
app.post('/api/mod-config', authPanel, async (req, res) => {
...
app.post('/api/orders', authPanel, async (req, res) => {
  ...
  // Livraison automatique si template configuré
  if (order.discordId) {
    tryAutoDeliver(order).catch(() => {});
  }
...
// ── Helper livraison — envoyer un fichier en DM ───────────────
async function sendDeliveryToUser(order, template, guild) {
  if (!template || !order.discordId) return { ok: false, error: 'Template ou discordId manquant' };
  try {
    const user = await client.users.fetch(order.discordId).catch(() => null);
    if (!user) return { ok: false, error: 'Utilisateur Discord introuvable' };

    // Remplacer les variables dans le contenu
    let content = template.content || '';
    const vars = {
      '{acheteur}':    order.username || user.username,
      '{date}':        order.date || new Date().toLocaleDateString('fr-FR'),
      '{article}':     order.articleName || 'Article',
      '{prix}':        order.price || '—',
      '{commande_id}': order.id,
      '{serveur}':     guild?.name || 'NexusStore',
    };
    Object.entries(vars).forEach(([k, v]) => { content = content.split(k).join(v); });

    // Injecter les secretLines (comptes, clés, liens secrets)
    if (template.secretLines?.length) {
      content += '\n\n━━━━━━━━━━━━━━━━━━━━━━━━━\n';
      content += '🔐 ACCÈS / LIVRAISON\n';
      content += '━━━━━━━━━━━━━━━━━━━━━━━━━\n';
      template.secretLines.forEach(line => {
    ...
    if (template.watermark) {
      content += `\n\n🔒 Livré à : ${order.username} (${order.discordId})\n📅 Date : ${order.date}\n🔖 Commande : ${order.id}`;
    }

    const fmt  = template.format || 'txt';
    const mime = fmt === 'html' ? 'text/html' : fmt === 'md' ? 'text/markdown' : 'text/plain';
    const buf  = Buffer.from(content, 'utf-8');
    const filename = `livraison-${order.id}.${fmt}`;
    const attachment = new AttachmentBuilder(buf, { name: filename, contentType: mime });

    // Message DM
    const dmMsg = (template.dm || '✅ Merci pour ton achat ! Voici ta livraison 📦')
      .replace(/{acheteur}/g, order.username)
      .replace(/{article}/g, order.articleName)
      .replace(/{commande_id}/g, order.id);

    // Embed de livraison
    const delivEmbed = new EmbedBuilder()
      .setTitle('📦 Ta livraison est prête !')
      .setColor(C('#00e5a0'))
      ...
      .setFooter({ text: 'Pour toute question → ouvre un ticket Discord' })
      .setTimestamp();

    // Bouton avis après livraison
    const reviewRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId('rev_' + order.id)
        .setLabel('⭐ Laisser un avis')
        .setStyle(ButtonStyle.Primary)
    );
    await user.send({ embeds: [delivEmbed], files: [attachment], components: [reviewRow] });

    // Poster aussi dans le ticket si option activée
    if (template.ticket && db.ticketMap) {
      const ticketChId = db.ticketMap[order.discordId];
      if (ticketChId) {
        const ticketCh = guild?.channels.cache.get(ticketChId);
        if (ticketCh) {
          const buf2 = Buffer.from(content, 'utf-8');
          await ticketCh.send({
            content: `📦 Livraison pour <@${order.discordId}> — Commande **${order.id}**`,
            files: [new AttachmentBuilder(buf2, { name: filename, contentType: mime })],
          }).catch(() => {});
        }
      }
    }

    return { ok: true, filename, username: user.username };
  } catch (e) {
    return { ok: false, error: e.message };
  }
}

// ── Route : livraison manuelle depuis le panel ────────────────
app.post('/api/orders/:id/deliver', authPanel, async (req, res) => {
  try {
    const order = db.orders.find(o => o.id === req.params.id);
    if (!order) return res.status(404).json({ error: 'Commande introuvable' });

    const { templateName, customContent, format, dm, watermark, ticket } = req.body;
    const guild = client.guilds.cache.get(GUILD_ID);

    // Utiliser un template prédéfini ou le contenu custom du panel
    let template = { content: customContent || '', format: format || 'txt', dm, watermark, ticket };

    // Si templateName fourni, le chercher dans la config
    if (templateName && db.deliveryTemplates) {
      const found = db.deliveryTemplates.find(t => t.name === templateName);
      if (found) template = { ...found, ...req.body };
    }

    if (!template.content) return res.status(400).json({ error: 'Contenu de livraison requis' });

    const result = await sendDeliveryToUser(order, template, guild);
    if (!result.ok) return res.status(500).json({ error: result.error });

    // Marquer comme livré
    order.status      = 'delivered';
    order.deliveredAt = new Date().toISOString();
    order.deliveryFile = result.filename;
    await SB.saveOrder(order);

    log(`📦 Livraison manuelle envoyée — ${order.id} → ${result.username}`);
    res.json({ ok: true, filename: result.filename, username: result.username });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});

// ── Route : modifier statut + livraison auto si delivered ─────
app.put('/api/orders/:id', authPanel, async (req, res) => {
  const order = db.orders.find(o => o.id === req.params.id);
  if (!order) return res.status(404).json({ error: 'Commande introuvable' });

  const { status, notes, autoDeliver, templateName, customContent, format, dm, watermark, ticket } = req.body;
  const guild = client.guilds.cache.get(GUILD_ID);

  if (status) order.status = status;
  if (notes !== undefined) order.notes = notes;

  // Si statut → delivered ET autoDeliver activé → envoyer la livraison
  if (status === 'delivered' && autoDeliver && order.discordId) {
    const template = {
      content:   customContent || db.deliveryTemplates?.find(t => t.name === templateName)?.content || '',
      format:    format || 'txt',
      dm:        dm || '✅ Ta commande est livrée ! Voici ton fichier 📦',
    ...
    if (template.content) {
      ...
      if (result.ok) {
        order.deliveryFile = result.filename;
        order.deliveredAt  = new Date().toISOString();
        log(`📦 Livraison auto — ${order.id} → ${result.username}`);
      }
    }
  }

  // Notifier le client en DM (statut uniquement, sans fichier)
  if (status && order.discordId && status !== 'delivered') {
    const statusLabel = {
      pending:   '⏳ En attente',
      confirmed: '✅ Confirmée',
      delivered: '📦 Livrée !',
...
// ── Sauvegarder les templates de livraison ────────────────────
app.get('/api/delivery-templates', authPanel, (req, res) => {
  res.json({ ok: true, templates: db.deliveryTemplates || [] });
});

app.post('/api/delivery-templates', authPanel, async (req, res) => {
  if (!db.deliveryTemplates) db.deliveryTemplates = [];
  const { name, content, format, dm, watermark, ticket, articleId, articleName, secretLines, auto } = req.body;
  if (!name) return res.status(400).json({ error: 'name requis' });

  const existing = db.deliveryTemplates.findIndex(t => t.name === name);
  const tpl = {
    name,
    content:     content     || '',
    format:      format      || 'txt',
    dm:          dm          || '✅ Merci pour ton achat ! Voici ta livraison 📦',
    watermark:   !!watermark,
    ticket:      !!ticket,
    auto:        !!auto,
    articleId:   articleId   || null,  // ID article lié (null = tous)
    articleName: articleName || '',    // Nom article pour affichage
    secretLines: secretLines || [],    // [{label, value}] — comptes, clés, liens
    savedAt:     new Date().toISOString(),
  };
  if (existing >= 0) db.deliveryTemplates[existing] = tpl;
  else db.deliveryTemplates.push(tpl);
  await SB.saveConfig('deliveryTemplates', db.deliveryTemplates);
  log(`📄 Template livraison sauvegardé : ${name} ${articleName ? '→ ' + articleName : '(global)'}`);
  res.json({ ok: true, template: tpl });
});

app.delete('/api/delivery-templates/:name', authPanel, async (req, res) => {
  if (!db.deliveryTemplates) return res.status(404).json({ error: 'Introuvable' });
  const idx = db.deliveryTemplates.findIndex(t => t.name === decodeURIComponent(req.params.name));
  if (idx < 0) return res.status(404).json({ error: 'Template introuvable' });
  const deleted = db.deliveryTemplates.splice(idx, 1)[0];
  await SB.saveConfig('deliveryTemplates', db.deliveryTemplates);
  log(`🗑️ Template livraison supprimé : ${deleted.name}`);
  res.json({ ok: true });
});

// ── Catalogue Discord ─────────────────────────────────────────
app.post('/api/catalogue', authPanel, async (req, res) => {
  try {
    const { channelId, mention, shopUrl, showStock, showLink } = req.body;
    ...
    const arts = db.articles.filter(a => a.visible !== false);
    if (!arts.length) return res.status(400).json({ error: 'Aucun article dans la boutique' });
    ...
    const mainEmb = new EmbedBuilder()
      .setTitle('🛒 Catalogue NexusStore')
      .setColor(C('#7c6dfa'))
      .setDescription(
        `Bienvenue dans notre boutique !\n` +
        (shopUrl ? `🌐 **[Voir la boutique en ligne](${shopUrl})**\n` : '') +
        `\n📌 Pour commander : ouvre un **ticket** ou contacte un staff.`
      )
      .setFooter({ text: `${arts.length} article${arts.length > 1 ? 's' : ''} · NexusStore` })
      .setTimestamp();

    if (shopUrl) mainEmb.setURL(shopUrl);

    // Ajouter les articles par catégorie (max 25 fields Discord, max 1024 chars par field)
    const catEntries = Object.entries(byCategory).slice(0, 24);
    for (const [cat, items] of catEntries) {
      let lines = items.map(a => {
        const stk = a.stock === -1 ? '♾️' : a.stock === 0 ? '❌ Rupture' : `✅ ${a.stock}`;
        const stockStr = showStock !== false ? ` · ${stk}` : '';
        const linkStr  = showLink && a.link ? ` · [Lien](${a.link})` : '';
        return `${a.emoji || '🛒'} **${a.name}** — \`${a.price}\`${stockStr}${linkStr}`;
      }).join('\n');
      // Tronquer si dépassement limite Discord
      if (lines.length > 1020) lines = lines.substring(0, 1017) + '...';
      mainEmb.addFields({ name: `${cat}`, value: lines || '—', inline: false });
    }
    ...
    if (shopUrl) {
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setLabel('🌐 Voir NexusStore').setURL(shopUrl).setStyle(ButtonStyle.Link)
      );
      components.push(row);
    }

    await ch.send({ content: mention || undefined, embeds: [mainEmb], components });
    log(`🛒 Catalogue publié dans #${ch.name} (${arts.length} articles)`);
    res.json({ ok: true, count: arts.length, channel: ch.name });
  } catch(e) {
    log(`❌ Erreur catalogue: ${e.message}`);
    res.status(500).json({ error: e.message });
  }
});

// ── Livraison automatique à la création d'une commande ────────
async function tryAutoDeliver(order) {
  if (!db.deliveryTemplates?.length) return;
  const guild = client.guilds.cache.get(GUILD_ID);
  // Chercher un template qui correspond à l'article ou un template global
  const tpl = db.deliveryTemplates.find(t => t.article == order.articleId)
           || db.deliveryTemplates.find(t => !t.article && t.auto);
  if (!tpl) return;
  const result = await sendDeliveryToUser(order, tpl, guild);
  if (result.ok) {
    order.status      = 'delivered';
    order.deliveryFile = result.filename;
    order.deliveredAt  = new Date().toISOString();
    await SB.saveOrder(order);
    log(`📦 Livraison automatique — ${order.id} → ${result.username}`);
  }
}
...
app.post('/api/about', authPanel, async (req, res) => {
...
// ── URL du panel (pour auto-connexion) ──
app.get('/api/panel-url', async (req, res) => {
  res.json({ url: global.PANEL_URL || null, ok: !!global.PANEL_URL });
});
...
// ── Panel : redirection vers GitHub Pages ──
app.get('/panel', (req, res) => {
  res.redirect('https://brian10122000.github.io/Nexusbot1/nexusbot-panel.html');
});
app.get('/admin/nexusbot-panel.html', (req, res) => {
  res.redirect('https://brian10122000.github.io/Nexusbot1/nexusbot-panel.html');
});


// ── Notification avis depuis le site web ──
app.post('/api/review-notify', async (req, res) => {
  const { name, stars, comment, product, date, source } = req.body;
  if (!name || !comment) return res.status(400).json({ error: 'name et comment requis' });

  // Sauvegarder l'avis dans la DB
  if (!db.reputation) db.reputation = [];
  const review = {
    id:          'REV-' + Date.now(),
    userId:      'web_user',
    username:    name,
    note:        stars || 5,
    stars:       '⭐'.repeat(Math.min(5, stars||5)),
    comment,
    articleName: product || '—',
  ...
  // Poster dans salon avis Discord
  try {
    const guild = client.guilds.cache.get(GUILD_ID);
    const revCh = guild?.channels.cache.get(CH_REVIEWS);
    if (revCh) {
      const col = (stars||5) >= 4 ? 0x00e5a0 : (stars||5) >= 3 ? 0xfbbf24 : 0xff4d4d;
      await revCh.send({ embeds: [new EmbedBuilder()
        .setTitle('⭐ Nouvel avis — Site web')
        .setColor(col)
        .setDescription('> ' + comment)
        .addFields(
          { name: '👤 Client',  value: name,                                      inline: true },
          { name: '⭐ Note',    value: '⭐'.repeat(Math.min(5,stars||5)) + ' (' + (stars||5) + '/5)', inline: true },
          { name: '📦 Article', value: product || '—',                            inline: true },
          { name: '🌐 Source',  value: source || 'Site web',                      inline: true },
        )
        .setFooter({ text: 'NexusStore — Avis vérifié ✅' })
        .setTimestamp()
      ]});
    }
  } catch(e) { log('⚠️ Review notify Discord: ' + e.message); }
...
// Tunnel public automatique via serveo.net (SSH)
const { execSync, spawn } = require('child_process');
setTimeout(() => {
  try {
    // Essayer serveo.net via SSH
    ...
    ssh.stdout.on('data', (data) => {
      ...
      if (txt.includes('https://')) {
        ...
        if (url) {
          ...
          console.log('🌐 ===== METS CETTE URL DANS LE PANEL =====');
          console.log('🌐', url);
          console.log('🌐 ==========================================');
        }
      }
    ...
    ssh.stderr.on('data', (data) => {
      const txt = data.toString();
      if (txt.includes('https://') || txt.includes('Forwarding')) {
        const url = txt.match(/https:\/\/[^\s]+/)?.[0];
        if (url) {
          console.log('🌐 URL PANEL:', url);
          console.log('🌐 ===== METS CETTE URL DANS LE PANEL =====');
    ...
    ssh.on('error', (e) => console.log('⚠️ SSH tunnel:', e.message));
  } catch(e) {
    console.log('⚠️ Tunnel:', e.message);
  }
}, 5000);
    if (!db.warns[t.id]) db.warns[t.id] = [];
...
// ══ TUNNEL PUBLIC AUTOMATIQUE ══
// Crée une URL publique pour accéder au panel depuis n'importe où
(async () => {
  // Attendre que le serveur soit bien démarré
  await new Promise(r => setTimeout(r, 2000));
  
  // Méthode 1 : localtunnel
  try {
    const lt = require('localtunnel');
    const tunnel = await lt({ port: PORT });
    global.PANEL_URL = tunnel.url + '/panel';
    // Sauvegarder l'URL dans Supabase pour que le panel puisse la lire
    try {
      await supabase.from('nexusbot_data').upsert({ key: 'panel_url', value: JSON.stringify({ url: global.PANEL_URL, updated: new Date().toISOString() }), updated_at: new Date().toISOString() });
    } catch(e) {}
    log('🌐 ══════════════════════════════════════════');
    log('🌐 URL PANEL : ' + global.PANEL_URL);
    log('🌐 ══════════════════════════════════════════');
    tunnel.on('close', () => { log('⚠️ Tunnel fermé — relance automatique dans 5s'); setTimeout(() => process.exit(1), 5000); });
    tunnel.on('error', () => {});
    return;
  } catch(e) {}

  // Méthode 2 : message direct
  log('🌐 Panel accessible via : http://localhost:' + PORT + '/admin/nexusbot-panel.html');
  log('💡 Pour accès externe, utilise GitHub Pages avec nexusbot-panel.html');
})();
...
// ════════════════════════════════════════
//  NEXUSBOT v12 — MINI-JEUX + PAIEMENTS
// ════════════════════════════════════════

// ─── In-memory stores ───
let gameStats = {}; // userId -> {wins,losses,games}
let lotteryPool = 0;
let lotteryTickets = []; // [userId]

// ─── COMMANDS SLASH ───

// /duel @joueur [mise]
if (process.env.GAME_DUEL !== 'false') {
  client.on('interactionCreate', async interaction => {
    if (!interaction.isChatInputCommand()) return;
    if (interaction.commandName !== 'duel') return;
    const target = interaction.options.getUser('joueur');
    const mise   = interaction.options.getInteger('mise') || 50;
    if (!target || target.bot) return interaction.reply({ content: '❌ Cible invalide', ephemeral: true });
    if (target.id === interaction.user.id) return interaction.reply({ content: '❌ Tu ne peux pas te battre toi-même !', ephemeral: true });

    // Check points
    const [att, def] = await Promise.all([
      supabase.from('economy').select('*').eq('user_id', interaction.user.id).single(),
      supabase.from('economy').select('*').eq('user_id', target.id).single()
    ]);
    if ((att.data?.points || 0) < mise) return interaction.reply({ content: `❌ Tu n'as pas assez de points (${att.data?.points || 0} < ${mise})`, ephemeral: true });
    if ((def.data?.points || 0) < mise) return interaction.reply({ content: `❌ ${target.username} n'a pas assez de points`, ephemeral: true });

    const embed = new EmbedBuilder()
      .setTitle('⚔️ DUEL')
      .setDescription(`**${interaction.user.username}** défie **${target.username}** pour **${mise} points** !\n\n${target.username}, tu as 30 secondes pour accepter avec ✅ ou refuser avec ❌`)
      .setColor('#ff4d00');
    const row = new ActionRowBuilder().addComponents(
    ...
    const filter = i => (i.customId === 'duel_accept' || i.customId === 'duel_refuse') && i.user.id === target.id;
    try {
      const collected = await msg.awaitMessageComponent({ filter, time: 30000 });
      if (collected.customId === 'duel_refuse') {
        return collected.update({ embeds: [embed.setDescription('❌ Duel refusé !')], components: [] });
      }
      // Fight!
      const attRoll = Math.floor(Math.random() * 100) + 1;
      const defRoll = Math.floor(Math.random() * 100) + 1;
      const winner  = attRoll > defRoll ? interaction.user : target;
      const loser   = attRoll > defRoll ? target : interaction.user;
      await supabase.from('economy').upsert({ user_id: winner.id, points: (attRoll > defRoll ? (att.data?.points||0) : (def.data?.points||0)) + mise });
      await supabase.from('economy').upsert({ user_id: loser.id,  points: Math.max(0, (attRoll > defRoll ? (def.data?.points||0) : (att.data?.points||0)) - mise) });
      const result = new EmbedBuilder()
        .setTitle('⚔️ Résultat du duel !')
        .setDescription(`🎲 **${interaction.user.username}** : **${attRoll}**\n🎲 **${target.username}** : **${defRoll}**\n\n🏆 **${winner.username}** remporte **${mise} points** !`)
        .setColor(attRoll > defRoll ? '#34d399' : '#fb7185');
      await collected.update({ embeds: [result], components: [] });
    } catch { await msg.edit({ embeds: [embed.setDescription('⏰ Duel expiré !')], components: [] }); }
  });
}
...
client.on('interactionCreate', async interaction => {
  if (!interaction.isChatInputCommand()) return;
  if (interaction.commandName !== 'loterie') return;
  const ticketCost = 10;
  const eco = await supabase.from('economy').select('*').eq('user_id', interaction.user.id).single();
  if ((eco.data?.points || 0) < ticketCost) return interaction.reply({ content: `❌ Tu n'as pas assez de points (${ticketCost} requis)`, ephemeral: true });
  await supabase.from('economy').upsert({ user_id: interaction.user.id, points: (eco.data.points - ticketCost) });
  lotteryPool  += ticketCost;
  lotteryTickets.push(interaction.user.id);
  await interaction.reply({ content: `🎰 Ticket acheté ! Pot actuel : **${lotteryPool} points**. Tirage quotidien automatique. Bonne chance !`, ephemeral: true });
});

// /pfc (pierre-feuille-ciseaux)
client.on('interactionCreate', async interaction => {
  if (!interaction.isChatInputCommand()) return;
  if (interaction.commandName !== 'pfc') return;
  const choices = ['✊ Pierre', '📄 Feuille', '✂️ Ciseaux'];
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('pfc_0').setLabel('✊').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('pfc_1').setLabel('📄').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('pfc_2').setLabel('✂️').setStyle(ButtonStyle.Secondary)
  );
  const msg = await interaction.reply({ content: '🎮 **Chifoumi** — Choisis !', components: [row], fetchReply: true });
  const filter = i => i.customId.startsWith('pfc_') && i.user.id === interaction.user.id;
  try {
    const collected = await msg.awaitMessageComponent({ filter, time: 15000 });
    const userChoice = parseInt(collected.customId.split('_')[1]);
    const botChoice  = Math.floor(Math.random() * 3);
    const wins  = [0,1,2]; // stone beats scissors(2), paper(1) beats stone(0), scissors(2) beats paper(1)
    const beats  = [2, 0, 1];
    let result;
    if (userChoice === botChoice) result = '🤝 Égalité !';
    else if (beats[userChoice] === botChoice) {
      result = `🏆 Tu gagnes ! (+25pts)`;
      const eco = await supabase.from('economy').select('points').eq('user_id', interaction.user.id).single();
      await supabase.from('economy').upsert({ user_id: interaction.user.id, points: (eco.data?.points||0)+25 });
    } else result = '😢 Tu perds !';
    await collected.update({ content: `**Toi :** ${choices[userChoice]}\n**Bot :** ${choices[botChoice]}\n\n${result}`, components: [] });
  } catch { await msg.edit({ content: '⏰ Temps écoulé !', components: [] }); }
});

// /des [mise]
client.on('interactionCreate', async interaction => {
  if (!interaction.isChatInputCommand()) return;
  if (interaction.commandName !== 'des') return;
  const mise = Math.min(200, Math.max(5, interaction.options.getInteger('mise') || 10));
  const eco  = await supabase.from('economy').select('*').eq('user_id', interaction.user.id).single();
  if ((eco.data?.points||0) < mise) return interaction.reply({ content: `❌ Pas assez de points`, ephemeral: true });
  const roll  = Math.floor(Math.random() * 6) + 1;
  const faces = ['', '⚀', '⚁', '⚂', '⚃', '⚄', '⚅'];
  const win   = roll >= 4;
  const delta = win ? mise : -mise;
  await supabase.from('economy').upsert({ user_id: interaction.user.id, points: (eco.data.points + delta) });
  await interaction.reply({ content: `🎲 **${faces[roll]}** (${roll}/6)\n${win ? `🏆 Tu gagnes **+${mise}pts** !` : `😢 Tu perds **${mise}pts**.`}` });
});

// Lottery daily auto-draw
setInterval(async () => {
  if (lotteryTickets.length < 2) return;
  if (!process.env.CH_SALES) return;
  const winner = lotteryTickets[Math.floor(Math.random() * lotteryTickets.length)];
  const eco    = await supabase.from('economy').select('points').eq('user_id', winner).single();
  await supabase.from('economy').upsert({ user_id: winner, points: (eco.data?.points||0) + lotteryPool });
  const ch = client.channels.cache.get(process.env.CH_SALES);
  if (ch) await ch.send({ embeds: [new EmbedBuilder().setTitle('🎰 JACKPOT !').setDescription(`<@${winner}> remporte **${lotteryPool} points** ! 🎉`).setColor('#fbbf24')] });
  lotteryPool = 0; lotteryTickets = [];
}, 24 * 60 * 60 * 1000); // daily

// ─── REGISTER NEW SLASH COMMANDS ───
const newCommands = [
  { name:'duel', description:'Défie un membre', options:[{name:'joueur',description:'@membre',type:6,required:true},{name:'mise',description:'Points à miser',type:4,required:false}] },
  { name:'loterie', description:'Acheter un ticket de loterie' },
  { name:'pfc', description:'Pierre-Feuille-Ciseaux contre le bot' },
  { name:'des', description:'Lance un dé', options:[{name:'mise',description:'Points',type:4,required:false}] }
];
// These will be registered on next bot deploy via /register route
app.post('/api/register-game-commands', authPanel, async (req, res) => {
  // Commandes déjà enregistrées via register() au démarrage
  res.json({ ok: true, registered: 0, message: 'Commandes gérées au démarrage' });
});

// ════════════════════════════════════════════════════════
//  ROUTES RÉELLES — PAIEMENTS, JEUX, QUIZ, PROFIL, ALERTES
// ════════════════════════════════════════════════════════

// ── DB extensions (in-memory, sauvegardées sur Supabase via nexusbot_data) ──
if (!db.payments)      db.payments      = []; // [{ id, client, userId, amount, method, articleId, articleName, status, date, note }]
if (!db.gameStats)     db.gameStats     = {}; // { userId: { wins, losses, games, pts_won, pts_lost } }
if (!db.quizQuestions) db.quizQuestions = []; // [{ id, question, answer, wrong[], pts, cat }]
if (!db.vendorProfile) db.vendorProfile = { name:'NexusStore', slogan:'', desc:'', color:'#5865f2', emoji:'⚡', invite:'', twitter:'', tiktok:'', insta:'', showReviews:true, showArts:true };
if (!db.alertConfig)   db.alertConfig   = { discord:true, channelId:'', low:5, crit:2, rupture:true, restock:true, msg:'' };
if (!db.alertHistory)  db.alertHistory  = []; // [{ type, article, stock, date }]
if (!db.lotteryPool)   db.lotteryPool   = 0;
if (!db.lotteryTickets)db.lotteryTickets= [];

// Helper: save extra data to Supabase nexusbot_data table
async function saveExtra(key, value) {
  try {
    await supabase.from('nexusbot_data').upsert({ key, value: JSON.stringify(value), updated_at: new Date().toISOString() });
  } catch(e) { console.error('saveExtra error:', e.message); }
}
async function loadExtra(key) {
  try {
    const { data } = await supabase.from('nexusbot_data').select('value').eq('key', key).single();
    return data ? JSON.parse(data.value) : null;
  } catch { return null; }
}

// Load all extra data on startup
(async () => {
  const [pays, gstats, quiz, vprof, acfg, ahist] = await Promise.all([
    loadExtra('payments'), loadExtra('gameStats'), loadExtra('quizQuestions'),
    loadExtra('vendorProfile'), loadExtra('alertConfig'), loadExtra('alertHistory')
  ]);
  if (pays)   db.payments      = pays;
  if (gstats) db.gameStats     = gstats;
  if (quiz)   db.quizQuestions = quiz;
  if (vprof)  db.vendorProfile = { ...db.vendorProfile, ...vprof };
  if (acfg)   db.alertConfig   = { ...db.alertConfig, ...acfg };
  if (ahist)  db.alertHistory  = ahist;
  console.log(`✅ Extras chargés : ${db.payments.length} paiements, ${db.quizQuestions.length} questions quiz`);
})();

// ── PAIEMENTS ──
app.get('/api/payments', authPanel, (req, res) => {
  // Merge avec commandes livrées pour avoir tout au même endroit
  const fromOrders = db.orders
    .filter(o => o.status === 'delivered')
    .map(o => ({
      id: 'ORD-' + o.id,
      client: o.username || o.userId,
      userId: o.userId,
      amount: parseFloat((o.price || '0').toString().replace(/[^0-9.]/g, '')) || 0,
      method: 'discord',
  ...
  // Merge : payments manuels + dérivés des commandes (sans doublons)
  const orderIds = new Set(fromOrders.map(o => o.id));
  const manual = db.payments.filter(p => !orderIds.has(p.id));
  const all = [...manual, ...fromOrders].sort((a, b) => new Date(b.date) - new Date(a.date));
  res.json(all);
});

app.post('/api/payments', authPanel, async (req, res) => {
  const { client, userId, amount, method, articleId, status, note } = req.body;
  if (!client || !amount) return res.status(400).json({ error: 'client et amount requis' });
  ...
  await saveExtra('payments', db.payments);
  res.json({ ok: true, payment: p });
});

app.delete('/api/payments/:id', authPanel, async (req, res) => {
...
// ── STATS JEUX ──
app.get('/api/game-stats', authPanel, (req, res) => {
  // Build leaderboard from gameStats
  const guild = client.guilds.cache.first();
  const rows = Object.entries(db.gameStats).map(([userId, s]) => {
...
app.post('/api/game-stats/reset', authPanel, async (req, res) => {
  db.gameStats = {};
  await saveExtra('gameStats', db.gameStats);
  res.json({ ok: true });
});

// ── QUIZ QUESTIONS ──
app.get('/api/quiz', authPanel, (req, res) => res.json(db.quizQuestions));

app.post('/api/quiz', authPanel, async (req, res) => {
  const { question, answer, wrong, pts, cat } = req.body;
  if (!question || !answer) return res.status(400).json({ error: 'question et answer requis' });
  const q = { id: Date.now(), question, answer, wrong: wrong || [], pts: pts || 50, cat: cat || '' };
  db.quizQuestions.unshift(q);
  await saveExtra('quizQuestions', db.quizQuestions);
  res.json({ ok: true, question: q });
});

app.delete('/api/quiz/:id', authPanel, async (req, res) => {
  db.quizQuestions = db.quizQuestions.filter(q => String(q.id) !== req.params.id);
  await saveExtra('quizQuestions', db.quizQuestions);
  res.json({ ok: true });
});

// ── PROFIL VENDEUR ──
app.get('/api/vendor-profile', authPanel, (req, res) => res.json(db.vendorProfile));

app.post('/api/vendor-profile', authPanel, async (req, res) => {
  db.vendorProfile = { ...db.vendorProfile, ...req.body };
  await saveExtra('vendorProfile', db.vendorProfile);
  res.json({ ok: true, profile: db.vendorProfile });
});

// ── CONFIG ALERTES STOCK ──
app.get('/api/alert-config', authPanel, (req, res) => res.json(db.alertConfig));

app.post('/api/alert-config', authPanel, async (req, res) => {
  db.alertConfig = { ...db.alertConfig, ...req.body };
  await saveExtra('alertConfig', db.alertConfig);
  res.json({ ok: true });
});

app.get('/api/alert-history', authPanel, (req, res) => res.json(db.alertHistory));

// ── LOTERIE — état réel ──
app.get('/api/lottery', authPanel, (req, res) => {
  res.json({ pool: db.lotteryPool, tickets: db.lotteryTickets.length });
});

app.post('/api/lottery/draw', authPanel, async (req, res) => {
  if (db.lotteryTickets.length < 1) return res.status(400).json({ error: 'Aucun ticket' });
  const winnerId = db.lotteryTickets[Math.floor(Math.random() * db.lotteryTickets.length)];
  const eco = db.economy[winnerId] || { points: 0 };
  eco.points = (eco.points || 0) + db.lotteryPool;
  db.economy[winnerId] = eco;
  await supabase.from('economy').upsert({ user_id: winnerId, points: eco.points });
  const guild = client.guilds.cache.first();
  const member = guild?.members.cache.get(winnerId);
  const ch = guild?.channels.cache.get(process.env.CH_SALES);
  if (ch) {
    await ch.send({ embeds: [new EmbedBuilder()
      .setTitle('🎰 JACKPOT LOTERIE !')
      .setDescription(`<@${winnerId}> remporte **${db.lotteryPool} points** ! 🎉`)
      .setColor('#fbbf24')
    ]});
  }
  const pot = db.lotteryPool;
  db.lotteryPool = 0;
  db.lotteryTickets = [];
  res.json({ ok: true, winner: member?.user.username || winnerId, pot });
});

// ── HOOK : enregistrer stats jeux après chaque partie ──
// Appelé depuis les commandes slash
function recordGameResult(userId, won, ptsChange) {
  if (!db.gameStats[userId]) db.gameStats[userId] = { wins:0, losses:0, games:0, pts_won:0, pts_lost:0 };
  db.gameStats[userId].games++;
  if (won) { db.gameStats[userId].wins++; db.gameStats[userId].pts_won += ptsChange || 0; }
  else      { db.gameStats[userId].losses++; db.gameStats[userId].pts_lost += Math.abs(ptsChange || 0); }
  saveExtra('gameStats', db.gameStats);
}

// ── HOOK : enregistrer alertes stock ──
function recordStockAlert(type, articleName, stock) {
  const entry = { type, article: articleName, stock, date: new Date().toISOString() };
  db.alertHistory.unshift(entry);
  if (db.alertHistory.length > 100) db.alertHistory.pop();
  saveExtra('alertHistory', db.alertHistory);
}

// ── CONFIG PAIEMENT (PayPal / Stripe) ──
if (!db.payConfig) db.payConfig = { paypal:'', paypalEmail:'', stripe:'', stripeSecret:'' };

// Charger config paiement au démarrage
(async () => {
  const cfg = await loadExtra('payConfig');
  if (cfg) db.payConfig = { ...db.payConfig, ...cfg };
})();

app.get('/api/pay-config', authPanel, (req, res) => {
  // Ne pas envoyer le stripeSecret au panel
  const { stripeSecret, ...safe } = db.payConfig;
  res.json({ ok:true, ...safe, hasStripeSecret: !!stripeSecret });
});

app.post('/api/pay-config', authPanel, async (req, res) => {
  const { paypal, paypalEmail, stripe, stripeSecret } = req.body;
  if (paypal !== undefined)       db.payConfig.paypal = paypal;
  if (paypalEmail !== undefined)  db.payConfig.paypalEmail = paypalEmail;
  if (stripe !== undefined)       db.payConfig.stripe = stripe;
  if (stripeSecret !== undefined) db.payConfig.stripeSecret = stripeSecret;
  // Mettre à jour aussi la boutique config legacy
  if (paypal)  db.boutiqueConfig.paypalLien = paypal;
  if (stripe)  db.boutiqueConfig.stripeLien = stripe;
  await saveExtra('payConfig', db.payConfig);
  res.json({ ok: true });
});

// ── EMBED DISCORD PAIEMENT ──
app.post('/api/pay-embed', authPanel, async (req, res) => {
  try {
    const { channelId, articleId, message, withPaypal, withStripe, paypalLink, stripeLink, articleName, articlePrice } = req.body;
    const channel = client.channels.cache.get(channelId);
    if (!channel) return res.status(404).json({ error: 'Salon introuvable' });

    const art = db.articles.find(a => String(a.id) === String(articleId));
    const name  = art?.name  || articleName  || 'Article';
    const price = art?.price || articlePrice || '—';
    const emoji = art?.emoji || '🛒';

    const embed = new EmbedBuilder()
      .setTitle(emoji + ' ' + name)
      .setDescription(message || 'Commande disponible !')
      .setColor(C('#5865f2'))
      .addFields(
        { name: '💰 Prix', value: '**' + price + '**', inline: true },
        { name: '📦 Livraison', value: 'Instantanée par DM', inline: true }
      )
      .setFooter({ text: 'NexusStore · Paiement 100% sécurisé 🔒' })
      .setTimestamp();

    const row = new ActionRowBuilder();
    if (withPaypal && paypalLink) {
      row.addComponents(new ButtonBuilder().setLabel('💳 Payer PayPal').setStyle(ButtonStyle.Link).setURL(paypalLink));
    }
    if (withStripe && stripeLink) {
      row.addComponents(new ButtonBuilder().setLabel('🔵 Payer Stripe').setStyle(ButtonStyle.Link).setURL(stripeLink));
    }
    // Bouton commande discord
    row.addComponents(new ButtonBuilder().setLabel('📋 Commander').setStyle(ButtonStyle.Primary).setCustomId('order_' + (art?.id || 'general')));
...
// ════════════════════════════════════════════════════════════════
//  SYSTÈME PAIEMENT & LIVRAISON AVANCÉ
//  PayPal.me auto + Stripe par article + livraison fichier ou info
// ════════════════════════════════════════════════════════════════

// ── Structure étendue des articles ──
// Chaque article peut avoir:
//   paypalUsername : string  → génère paypal.me/username/prix
//   stripeLink     : string  → lien Stripe Payment Link spécifique
//   deliveryType   : 'file' | 'info' | 'none'
//   deliveryContent: string  → contenu à livrer (texte/infos/clé)
//   deliveryFile   : string  → nom du fichier à envoyer (type 'file')
//   autoDeliver    : boolean → livraison auto à la commande

// ── Route : mettre à jour les infos paiement d'un article ──
app.post('/api/articles/:id/payment', authPanel, async (req, res) => {
  const art = db.articles.find(a => String(a.id) === req.params.id);
  if (!art) return res.status(404).json({ error: 'Article introuvable' });

  const { paypalUsername, stripeLink, deliveryType, deliveryContent, autoDeliver } = req.body;
  if (paypalUsername !== undefined) art.paypalUsername = paypalUsername;
  if (stripeLink     !== undefined) art.stripeLink     = stripeLink;
  if (deliveryType   !== undefined) art.deliveryType   = deliveryType;
  if (deliveryContent!== undefined) art.deliveryContent= deliveryContent;
  if (autoDeliver    !== undefined) art.autoDeliver     = autoDeliver;

  // Sauvegarder dans Supabase (colonne extras en JSON)
  try {
    await supabase.from('articles').update({
      paypal_username:  art.paypalUsername || null,
      stripe_link:      art.stripeLink     || null,
      delivery_type:    art.deliveryType   || 'none',
      delivery_content: art.deliveryContent|| null,
      auto_deliver:     art.autoDeliver    || false,
    }).eq('id', art.id);
  } catch(e) {
    // Fallback : sauvegarder dans nexusbot_data
    await saveExtra('art_pay_' + art.id, {
      paypalUsername: art.paypalUsername,
      stripeLink:     art.stripeLink,
      deliveryType:   art.deliveryType,
      deliveryContent:art.deliveryContent,
      autoDeliver:    art.autoDeliver,
    });
  }
  res.json({ ok: true, article: art });
});

// ── Route : générer un lien de paiement pour une commande ──
app.post('/api/orders/:id/payment-link', authPanel, async (req, res) => {
  const order = db.orders.find(o => String(o.id) === req.params.id);
  if (!order) return res.status(404).json({ error: 'Commande introuvable' });

  const art = db.articles.find(a => String(a.id) === String(order.articleId));
  const paypalUser = art?.paypalUsername || db.payConfig?.paypalUsername || '';
  const stripeLink = art?.stripeLink     || db.payConfig?.stripe         || '';

  // Extraire le montant numérique du prix
  const priceNum = parseFloat((order.price || '0').toString().replace(/[^0-9.,]/g, '').replace(',', '.')) || 0;

  const links = {};
  if (paypalUser && priceNum > 0) {
    links.paypal = `https://paypal.me/${paypalUser.replace('@','')}/${priceNum}EUR`;
  }
  if (stripeLink) {
    // Ajouter les métadonnées à l'URL Stripe si possible
    const url = new URL(stripeLink);
    url.searchParams.set('client_reference_id', order.id);
    links.stripe = url.toString();
  }

  res.json({ ok: true, links, order: { id: order.id, price: order.price, article: order.articleName } });
});

// ── Route : envoyer les liens de paiement par DM Discord ──
app.post('/api/orders/:id/send-payment', authPanel, async (req, res) => {
  const order = db.orders.find(o => String(o.id) === req.params.id);
  if (!order) return res.status(404).json({ error: 'Commande introuvable' });
  if (!order.discordId) return res.status(400).json({ error: 'Pas de Discord ID pour cette commande' });
  ...
  try {
    const user = await client.users.fetch(order.discordId).catch(() => null);
    if (!user) return res.status(404).json({ error: 'Utilisateur Discord introuvable' });

    const embed = new EmbedBuilder()
      .setTitle('💳 Paiement — ' + (order.articleName || 'Commande'))
      .setDescription(`Bonjour **${order.username || user.username}** !\n\nMerci pour ta commande. Voici les options de paiement pour finaliser :`)
      .setColor(C('#5865f2'))
      .addFields(
        { name: '🔖 Commande',  value: '`' + order.id + '`',           inline: true },
        { name: '📦 Article',   value: order.articleName || '—',        inline: true },
    ...
    if (paypalUser && priceNum > 0) {
      const ppLink = `https://paypal.me/${paypalUser.replace('@','')}/${priceNum}EUR`;
      row.addComponents(new ButtonBuilder().setLabel('💳 Payer via PayPal').setStyle(ButtonStyle.Link).setURL(ppLink));
      embed.addFields({ name: '💳 PayPal', value: `[**→ Payer ${order.price} via PayPal.me**](${ppLink})`, inline: false });
      hasButtons = true;
    }
    if (stripeLink) {
      const url = new URL(stripeLink);
      url.searchParams.set('client_reference_id', order.id);
      const stLink = url.toString();
      row.addComponents(new ButtonBuilder().setLabel('🔵 Payer via Stripe').setStyle(ButtonStyle.Link).setURL(stLink));
      embed.addFields({ name: '🔵 Stripe', value: `[**→ Payer par carte bancaire**](${stLink})`, inline: false });
      hasButtons = true;
    }

    if (!hasButtons) {
      embed.addFields({ name: '📩 Contact', value: 'Contacte-nous pour finaliser le paiement', inline: false });
    }

    await user.send({ embeds: [embed], components: hasButtons ? [row] : [] });
    order.status = 'confirmed';
    await SB.saveOrder(order);

    log(`💳 Lien paiement envoyé à ${user.username} pour ${order.id}`);
...
// ── Livraison améliorée : fichier OU info texte ──
// Remplacer sendDeliveryToUser pour gérer les 2 modes
const _origSendDelivery = sendDeliveryToUser;
async function sendDeliveryToUser(order, template, guild) {
  const art = db.articles.find(a => String(a.id) === String(order.articleId));

  // Mode INFO : livrer un texte/code/compte en DM (pas de fichier)
  if (art?.deliveryType === 'info' || template.deliveryType === 'info') {
    const user = await client.users.fetch(order.discordId).catch(() => null);
    if (!user) return { ok: false, error: 'Utilisateur introuvable' };

    const infoContent = art?.deliveryContent || template.deliveryContent || template.content || '';
    const resolvedContent = infoContent
      .replace(/{acheteur}/g,    order.username || user.username)
      .replace(/{date}/g,        order.date || new Date().toLocaleDateString('fr-FR'))
      .replace(/{article}/g,     order.articleName || 'Article')
      .replace(/{prix}/g,        order.price || '—')
      .replace(/{commande_id}/g, order.id);

    const embed = new EmbedBuilder()
      .setTitle('✅ Livraison — ' + (order.articleName || 'Commande'))
      .setDescription(template.dm || '✅ Voici ta livraison !')
      .setColor(C('#10d982'))
      .addFields({ name: '🔐 Accès / Informations', value: '```\n' + resolvedContent + '\n```', inline: false })
      .addFields({ name: '🔖 Commande', value: '`' + order.id + '`', inline: true })
      .setFooter({ text: 'NexusStore · Ne partage jamais ces informations' })
      .setTimestamp();

    await user.send({ embeds: [embed] });
    return { ok: true, filename: 'info_dm', username: user.username };
  }

  // Mode FICHIER : comportement original
  return _origSendDelivery(order, template, guild);
}

// ── tryAutoDeliver amélioré ──
const _origTryAutoDeliver = tryAutoDeliver;
async function tryAutoDeliver(order) {
  const art = db.articles.find(a => String(a.id) === String(order.articleId));

  // Si l'article a une livraison directe configurée
  if (art && art.autoDeliver && art.deliveryContent && art.deliveryType !== 'none') {
    const guild = client.guilds.cache.get(GUILD_ID);
    const fakeTemplate = {
      deliveryType:    art.deliveryType    || 'info',
      deliveryContent: art.deliveryContent || '',
      content:         art.deliveryContent || '',
      dm:              '✅ Ta commande **' + (art.name||'') + '** est livrée !',
      auto: true,
    };
    const result = await sendDeliveryToUser(order, fakeTemplate, guild);
    if (result.ok) {
      order.status      = 'delivered';
      order.deliveredAt = new Date().toISOString();
      await SB.saveOrder(order);
      log(`📦 Livraison auto (${art.deliveryType}) — ${order.id} → ${result.username}`);
    }
    return;
  }

  // Sinon comportement original (templates)
  return _origTryAutoDeliver(order);
}


// ── Route : récupérer toutes les configs paiement/livraison des articles ──
app.get('/api/art-dlv-configs', authPanel, async (req, res) => {
  const configs = {};
  for (const art of db.articles) {
    if (art.paypalUsername || art.stripeLink || art.deliveryType) {
      configs[art.id] = {
        paypalUsername:  art.paypalUsername  || '',
        stripeLink:      art.stripeLink      || '',
        deliveryType:    art.deliveryType    || 'none',
        deliveryContent: art.deliveryContent || '',
        autoDeliver:     art.autoDeliver     || false,
      };
    }
    // Essayer de charger depuis nexusbot_data si pas en mémoire
    else {
      const saved = await loadExtra('art_pay_' + art.id);
      if (saved) {
        Object.assign(art, saved);
        configs[art.id] = saved;
      }
    }
  }
  res.json(configs);
...
// ── Route : test de livraison par article ──
app.post('/api/delivery/test', authPanel, async (req, res) => {
  try {
    const { discordId, articleId } = req.body;
    ...
    const fakeOrder = {
      ...
      username:    'TestUser',
      articleId:   articleId,
      articleName: art?.name    || 'Article Test',
      price:       art?.price   || '9.99€',
      date:        new Date().toLocaleDateString('fr-FR'),
    };
    const fakeTemplate = {
      deliveryType:    art?.deliveryType    || 'info',
      deliveryContent: art?.deliveryContent || 'Ceci est un test de livraison.\nCommande : {commande_id}\nArticle : {article}',
      content:         art?.deliveryContent || 'Ceci est un test de livraison.',
      dm:              '🧪 Test de livraison — ' + (art?.name || 'Article'),
      auto: false,
    };
    const result = await sendDeliveryToUser(fakeOrder, fakeTemplate, guild);
    if (result.ok) res.json({ ok: true, username: result.username });
...
// ════════════════════════════════════════════════
//  STOCK PRIVÉ — API ROUTES
// ════════════════════════════════════════════════
if (!db.vault) db.vault = [];

// Charger le vault au démarrage
(async () => {
  const saved = await loadExtra('vault');
  if (saved) db.vault = saved;
  console.log(`🔐 Vault chargé : ${db.vault.length} éléments`);
})();

const saveVault = () => saveExtra('vault', db.vault);

// GET — liste tous les items (sans le contenu masqué)
app.get('/api/vault', authPanel, (req, res) => {
  const items = db.vault.map(item => ({
    id:         item.id,
    ...
    // Contenu visible seulement si demandé explicitement
...
app.post('/api/vault', authPanel, async (req, res) => {
  const { category, label, content, articleId, secret } = req.body;
  if (!category || !content) return res.status(400).json({ error: 'category et content requis' });
  const item = {
    id:        'VLT-' + Date.now(),
...
// POST bulk — ajouter plusieurs lignes d'un coup
app.post('/api/vault/bulk', authPanel, async (req, res) => {
  const { category, lines, articleId } = req.body;
  if (!category || !lines || !lines.length) return res.status(400).json({ error: 'category et lines requis' });
  const now = Date.now();
  ...
  db.vault.unshift(...newItems);
  await saveVault();
  res.json({ ok: true, added: newItems.length });
});

// POST — changer le statut (used/available)
app.post('/api/vault/:id/status', authPanel, async (req, res) => {
  const item = db.vault.find(i => i.id === req.params.id);
  if (!item) return res.status(404).json({ error: 'Item introuvable' });
  item.status = req.body.status || 'available';
  if (req.body.status === 'used') item.usedAt = new Date().toISOString();
  await saveVault();
  res.json({ ok: true });
});

// DELETE — supprimer un item
app.delete('/api/vault/:id', authPanel, async (req, res) => {
  const before = db.vault.length;
  db.vault = db.vault.filter(i => i.id !== req.params.id);
  if (db.vault.length === before) return res.status(404).json({ error: 'Item introuvable' });
  await saveVault();
  res.json({ ok: true });
});

// GET — prochain item disponible pour un article (pour livraison auto)
app.get('/api/vault/next/:articleId', authPanel, (req, res) => {
  const item = db.vault.find(i =>
    String(i.articleId) === req.params.articleId &&
    i.status === 'available'
  );
  if (!item) return res.status(404).json({ error: 'Aucun stock disponible pour cet article' });
  res.json({ ok: true, item });
});
