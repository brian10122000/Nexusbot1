...
app.post('/api/articles/:id/stock', authPanel, async (req, res) => {
  ...
  const nsWorkerUrl = process.env.NEXUSSTORE_WORKER_URL || '';
  const nsApiKey    = process.env.NEXUSSTORE_API_KEY || '';
  if (nsWorkerUrl) {
...
async function sendDeliveryToUser(order, template, guild) {
  ...
  try {
    ...
    await user.send({ embeds: [delivEmbed], files: [attachment] });
...
app.post('/api/register-game-commands', authPanel, async (req, res) => {
  try {
    ...
    const toAdd = newCommands.filter(nc => !existing.find(e => e.name === nc.name));
    for (const cmd of toAdd) {
      await rest.post(Routes.applicationGuildCommands(process.env.CLIENT_ID, process.env.GUILD_ID), { body: cmd });
    }
    res.json({ ok: true, registered: toAdd.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
