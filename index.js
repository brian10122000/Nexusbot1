...
app.post('/api/delivery-templates', authPanel, async (req, res) => {
  ...
  const { name, content, format, dm, watermark, ticket, article } = req.body;
  ...
  const tpl = { name, content, format: format || 'txt', dm: dm || '', watermark: !!watermark, ticket: !!ticket, article: article || '', savedAt: new Date().toISOString() };
  ...
  log(`📄 Template livraison sauvegardé : ${name}`);
...
app.delete('/api/delivery-templates/:name', authPanel, async (req, res) => {
  ...
  const idx = db.deliveryTemplates.findIndex(t => t.name === req.params.name);
  if (idx < 0) return res.status(404).json({ error: 'Template introuvable' });
  db.deliveryTemplates.splice(idx, 1);
  await SB.saveConfig('deliveryTemplates', db.deliveryTemplates);
  res.json({ ok: true });
});
