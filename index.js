...
app.post('/api/catalogue', authPanel, async (req, res) => {
  try {
    ...
    const mainEmb = new EmbedBuilder()
      ...
      .setColor(C('#7c6dfa'))
      .setDescription(
        `Bienvenue dans notre boutique ! Voici tous nos articles disponibles.\n` +
        (shopUrl ? `\n🌐 **[Voir la boutique en ligne](${shopUrl})**` : '') +
        `\n\n📌 Pour commander : ouvre un **ticket** ou contacte un staff.`
      )
      .setFooter({ text: `${arts.length} article${arts.length > 1 ? 's' : ''} disponible${arts.length > 1 ? 's' : ''} · NexusStore` })
      .setTimestamp();
    ...
    Object.entries(byCategory).forEach(([cat, items]) => {
      const lines = items.map(a => {
        const stk = a.stock === -1 ? '♾️' : a.stock === 0 ? '❌' : `✅ (${a.stock})`;
        const stockStr = showStock !== false ? ` · Stock : ${stk}` : '';
        const linkStr  = showLink && a.link ? ` · [Voir](${a.link})` : '';
        return `${a.emoji || '🛒'} **${a.name}** — \`${a.price}\`${stockStr}${linkStr}`;
      }).join('\n');
      mainEmb.addFields({ name: `📁 ${cat}`, value: lines, inline: false });
    });

    // Bouton lien boutique si URL fournie
    const components = [];
    if (shopUrl) {
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setLabel('🌐 Voir la boutique')
          .setURL(shopUrl)
